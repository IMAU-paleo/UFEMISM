MODULE ice_dynamics_module
  ...
  USE ice_velocity_module,             ONLY: solve_SIA, solve_SSA, solve_DIVA, initialise_velocity_solver, remap_velocity_solver
...
! == Administration: allocation, initialisation, and remapping
  ...
  SUBROUTINE remap_ice_model( mesh_old, mesh_new, map, ice, refgeo_PD, time)
    ...
    ! Velocity solver
    CALL remap_velocity_solver( mesh_old, mesh_new, map, ice)
   ...
   !CALL reallocate_shared_dp_2D(   mesh_new%nV      , C%nz       ,     ice%Ti_a                  , ice%wTi_a                 ) 
    ...
    CALL reallocate_shared_dp_1D(   mesh_new%nVAaAc  ,                  ice%f_grnd_ac             , ice%wf_grnd_ac            )
