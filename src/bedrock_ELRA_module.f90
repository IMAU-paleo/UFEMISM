...
CONTAINS
  ...
  SUBROUTINE run_ELRA_model( region)
    ...
    DO vi = region%mesh%vi1, region%mesh%vi2
      ...
      region%ice%dHb_a( vi) = region%ice%Hb_a( vi) - region%refgeo_GIAeq%Hb( vi)
    END DO
    CALL sync
  ...
  SUBROUTINE initialise_ELRA_model( mesh, grid, ice, refgeo_GIAeq)
    ...
    CALL initialise_ELRA_PD_reference_load( mesh, grid, ice, refgeo_GIAeq)

    ! Finalise routine path
    CALL finalise_routine( routine_name, n_extra_windows_expected=10)

  END SUBROUTINE initialise_ELRA_model
  SUBROUTINE initialise_ELRA_PD_reference_load( mesh, grid, ice, refgeo_GIAeq)
  ...
  SUBROUTINE remap_ELRA_model( mesh_old, mesh_new, map, ice, refgeo_GIAeq, grid)
    ...
    CALL initialise_ELRA_PD_reference_load( mesh_new, grid, ice, refgeo_GIAeq)
