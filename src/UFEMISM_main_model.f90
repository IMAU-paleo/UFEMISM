...
#       endif
      ...
      t1 = MPI_WTIME()
      CALL run_thermo_model( region%mesh, region%ice, region%climate_matrix%applied, region%SMB, region%BMB, region%time, do_solve_heat_equation = region%do_thermo)
      t2 = MPI_WTIME()
...
#     endif
  ...
  SUBROUTINE initialise_model( region, name, climate_matrix_global, ocean_matrix_global)
    ...
    ! Initialise the ice temperature profile
    CALL initialise_ice_temperature( region%mesh, region%ice, region%climate_matrix%applied, region%SMB, region%name, region%restart)
