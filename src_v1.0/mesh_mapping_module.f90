MODULE mesh_mapping_module
  ...
  USE mesh_help_functions_module,  ONLY: IsInTriangle, FindVoronoiCellVertices, FindContainingTriangle, Cross, Cross2, FindContainingVertex, &
                                         LineIntegral_xdy, LineIntegral_mxydx, LineIntegral_xydy, FindTriangleAreas, IsboundarySegment, &
                                         LiesOnLineSegment, SegmentIntersection, DetermineProcessXYDomain_balanced, WriteMeshToTextFile, &
                                         RedoTriEdgeIndices, CheckMesh, LineFromPoints, LineLineIntersection
  USE mesh_memory_module,          ONLY: AllocateMesh, CropMeshMemory, DeallocateSubmesh
  USE mesh_derivatives_module,     ONLY: GetMeshDerivatives, GetMeshDerivatives_3D
  USE mesh_ArakawaC_module,        ONLY: MakeAcMesh
  USE zeta_module,                 ONLY: p_zeta
  ...
  SUBROUTINE MapCart2Mesh(mesh, map_mean_i, map_mean_w, map_bilin_i, map_bilin_w, ny, i1, i2, cartdata, meshdata)
  ...
  END SUBROUTINE MapCart2Mesh
  SUBROUTINE MapCart2Mesh_3D(mesh, map_mean_i, map_mean_w, map_bilin_i, map_bilin_w, ny, nz, i1, i2, cartdata, meshdata)
    ! Map a single cartesian data field to the mesh, using the pre-calculated mapping arrays
  ...
  END SUBROUTINE MapCart2Mesh_3D
  SUBROUTINE GetCart2MeshMap(mesh,x,y,nx,ny, map_mean_i,  map_mean_w,  map_bilin_i,  map_bilin_w, &
    ...
    CALL allocate_shared_memory_int_2D(nx,      ny,     map_mean_i,   wmap_mean_i)
    CALL allocate_shared_memory_dp_2D( nx,      ny,     map_mean_w,   wmap_mean_w)
    CALL allocate_shared_memory_int_2D(mesh%nV, 4,      map_bilin_i,  wmap_bilin_i)
    CALL allocate_shared_memory_dp_2D( mesh%nV, 4,      map_bilin_w,  wmap_bilin_w)
  ...
  END SUBROUTINE GetCart2MeshMap
  SUBROUTINE GetGlob2MeshMap(mesh, lat,lon,nlat,nlon, map_mean_i,  map_mean_w,  map_bilin_i,  map_bilin_w, &
                                                     wmap_mean_i, wmap_mean_w, wmap_bilin_i, wmap_bilin_w)
    ...
    ! Allocate shared memory for the mapping arrays
    CALL allocate_shared_memory_int_2D(nlon,      nlat,     map_mean_i,   wmap_mean_i)
    CALL allocate_shared_memory_dp_2D( nlon,      nlat,     map_mean_w,   wmap_mean_w)
    CALL allocate_shared_memory_int_2D(mesh%nV,   4,        map_bilin_i,  wmap_bilin_i)
    CALL allocate_shared_memory_dp_2D( mesh%nV,   4,        map_bilin_w,  wmap_bilin_w)
  ...
  END SUBROUTINE GetGlob2MeshMap

  ! == Subroutines for remapping data between an old mesh and a new mesh ==
  SUBROUTINE CreateRemappingArrays( mesh_src, mesh_dst, map)
    ...
    CALL CreateRemappingArrays_trilin(            mesh_src, mesh_dst, map%trilin)
    CALL CreateRemappingArrays_nearest_neighbour( mesh_src, mesh_dst, map%nearest_neighbour)
    ...
    CALL allocate_shared_memory_dp_1D( mesh_dst%nV, A_src, wA_src)    
    CALL RemapMesh2Mesh_nearest_neighbour_2D( mesh_dst, map%nearest_neighbour, mesh_src%A, A_src)    
    nV_max = 0
    DO vi = 1, mesh_dst%nV
      nV_max = MAX(MAX( nV_max, CEILING( mesh_dst%A(vi) / A_src(vi))), CEILING( A_src(vi) / mesh_dst%A(vi)))
    END DO
    nV_max = nV_max * 3    
    CALL deallocate_shared_memory( wA_src)
    
    CALL CreateRemappingArrays_conservative(      mesh_src, mesh_dst, nV_max, map%conservative)
    
  END SUBROUTINE CreateRemappingArrays
  SUBROUTINE DeallocateRemappingArrays( map)
  
    TYPE(type_remapping),                    INTENT(INOUT) :: map
    
    CALL deallocate_shared_memory( map%trilin%wvi)
    CALL deallocate_shared_memory( map%trilin%ww)
    
    NULLIFY( map%trilin%vi)
    NULLIFY( map%trilin%w)    
    
    CALL deallocate_shared_memory( map%nearest_neighbour%wvi)
    
    NULLIFY( map%nearest_neighbour%vi)
    
    CALL deallocate_shared_memory( map%conservative%wnV)
    CALL deallocate_shared_memory( map%conservative%wvi)
    CALL deallocate_shared_memory( map%conservative%ww0)
    CALL deallocate_shared_memory( map%conservative%ww1x)
    CALL deallocate_shared_memory( map%conservative%ww1y)
  ...
  END SUBROUTINE DeallocateRemappingArrays
  
  SUBROUTINE CreateRemappingArrays_trilin( mesh_src, mesh_dst, map)
    ...
    CALL allocate_shared_memory_int_2D( mesh_dst%nV, 3, map%vi, map%wvi)
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nV, 3, map%w,  map%ww)
    ...
    DO vi_dst = mesh_dst%v1, mesh_dst%v2
      ...
      CALL FindContainingTriangle( mesh_src, p, ti_src)
  ...
  END SUBROUTINE CreateRemappingArrays_trilin
  SUBROUTINE CreateRemappingArrays_nearest_neighbour( mesh_src, mesh_dst, map)
    ...
    ! Allocate shared memory
    CALL allocate_shared_memory_int_1D( mesh_dst%nV, map%vi, map%wvi)
    ...
    DO vi_dst = mesh_dst%v1, mesh_dst%v2
      ...
      ! Find the mesh_src triangle containing this vertex
      CALL FindContainingVertex(mesh_src, p, vi_src)
  ...
  END SUBROUTINE CreateRemappingArrays_nearest_neighbour
  
  ! == Subroutines for conservative remapping
  SUBROUTINE CreateRemappingArrays_conservative( mesh_src, mesh_dst, nV_max, map)
    ...
    INTEGER,  DIMENSION(:  ), ALLOCATABLE                  :: proc_domain_V_src, proc_domain_Ac_src
    INTEGER,  DIMENSION(:  ), ALLOCATABLE                  :: proc_domain_V_dst, proc_domain_Ac_dst
    ...
    ! Save both meshes to text files, for debugging purposes
    IF (par%master) CALL WriteMeshToTextFile( mesh_src, '/mesh_src.txt')
    IF (par%master) CALL WriteMeshToTextFile( mesh_dst, '/mesh_dst.txt')
    
    ! Allocate memory for all the temporary data used in conservative remapping
    CALL CreateRemappingArrays_conservative_allocate_memory( mesh_src, mesh_dst, nV_max, &
      proc_domain_V_src, proc_domain_Ac_src, proc_domain_V_dst, proc_domain_Ac_dst, &
...
!    IF (par%master) WRITE(0,*) '  Remapping: find Voronoi boundary lines'
    
    CALL FindVoronoiBoundaryLines( mesh_src, Vor_lines_src, Vor_vi_ti_src)
    CALL FindVoronoiBoundaryLines( mesh_dst, Vor_lines_dst, Vor_vi_ti_dst)
    
!    IF (par%master) WRITE(0,*) '   Checksum: Vor_lines_src = ', SUM(Vor_lines_src), ' (should be 242094744.1506)'
!    IF (par%master) WRITE(0,*) '   Checksum: Vor_lines_dst = ', SUM(Vor_lines_dst), ' (should be -466949925.3714)'
!    IF (par%master) WRITE(0,*) '   Checksum: Vor_vi_ti_src = ', SUM(Vor_vi_ti_src), ' (should be 209205014)'
!    IF (par%master) WRITE(0,*) '   Checksum: Vor_vi_ti_dst = ', SUM(Vor_vi_ti_dst), ' (should be 203221477)'
    
    ! Determine process vertex domains for both meshes
  
    CALL DetermineProcessDomains( mesh_src, proc_domain_V_src, proc_domain_Ac_src, vi_src_start_proc)
    CALL DetermineProcessDomains( mesh_dst, proc_domain_V_dst, proc_domain_Ac_dst, vi_dst_start_proc)

    ! Integrate over all Voronoi cell boundary lines for both meshes
    
!    IF (par%master) WRITE(0,*) '  Remapping: integrate over mesh_src Voronoi boundary lines'
  
    CountCoincidences = .FALSE.  
    CALL IntegrateOverVoronoiBoundaries( mesh_src, mesh_dst, proc_domain_V_src, proc_domain_Ac_src, &
...
!    IF (par%master) THEN
!      WRITE(0,*) ' Checksum: r_Ac_src_nV           = ', SUM(r_Ac_src_nV          ), ' (should be 20086)'
!      WRITE(0,*) ' Checksum: r_Ac_src_vi_dst_left  = ', SUM(r_Ac_src_vi_dst_left ), ' (should be 42503370)'
!      WRITE(0,*) ' Checksum: r_Ac_src_vi_dst_right = ', SUM(r_Ac_src_vi_dst_right), ' (should be 42503370)'
!      WRITE(0,*) ' Checksum: r_Ac_src_LI_xdy       = ', SUM(r_Ac_src_LI_xdy      ), ' (should be -115797866648.6149)'
!    END IF
    
!    IF (par%master) WRITE(0,*) '  Remapping: integrate over mesh_dst Voronoi boundary lines'
  
    CountCoincidences = .TRUE.  
    CALL IntegrateOverVoronoiBoundaries( mesh_dst, mesh_src, proc_domain_V_dst, proc_domain_Ac_dst, &
...
!    IF (par%master) THEN
!      WRITE(0,*) ' Checksum: r_Ac_dst_nV           = ', SUM(r_Ac_dst_nV          ), ' (should be 20931)'
!      WRITE(0,*) ' Checksum: r_Ac_dst_vi_src_left  = ', SUM(r_Ac_dst_vi_src_left ), ' (should be 43570287)'
!      WRITE(0,*) ' Checksum: r_Ac_dst_vi_src_right = ', SUM(r_Ac_dst_vi_src_right), ' (should be 43851316)'
!      WRITE(0,*) ' Checksum: r_Ac_dst_LI_xdy       = ', SUM(r_Ac_dst_LI_xdy      ), ' (should be -521882144681.9128)'
!    END IF

    ! Rearrange integral contributions from Aci to vertices
    
!    IF (par%master) WRITE(0,*) '  Remapping: rearrange mesh_src integral data from Voronoi boundary lines to vertices'
  
    CALL RearrangeContributionsFromLinesToVertices( mesh_src, r_Ac_src_nV, r_Ac_src_vi_dst_left, r_Ac_src_vi_dst_right, &
      r_Ac_src_LI_xdy, r_Ac_src_LI_mxydx, r_Ac_src_LI_xydy, r_src_nV, r_src_vi_dst, r_src_LI_xdy, r_src_LI_mxydx, r_src_LI_xydy)
    
!    IF (par%master) THEN
!      WRITE(0,*) ' Checksum: r_src_nV     = ', SUM(r_src_nV    ), ' (should be 16446)'
!      WRITE(0,*) ' Checksum: r_src_vi_dst = ', SUM(r_src_vi_dst), ' (should be 34587441)'
!      WRITE(0,*) ' Checksum: r_src_LI_xdy = ', SUM(r_src_LI_xdy), ' (should be ~zero)'
!    END IF
!    CALL sync
    
!    IF (par%master) WRITE(0,*) '  Remapping: rearrange mesh_dst integral data from Voronoi boundary lines to vertices'
  
    CALL RearrangeContributionsFromLinesToVertices( mesh_dst, r_Ac_dst_nV, r_Ac_dst_vi_src_left, r_Ac_dst_vi_src_right, &
      r_Ac_dst_LI_xdy, r_Ac_dst_LI_mxydx, r_Ac_dst_LI_xydy, r_dst_nV, r_dst_vi_src, r_dst_LI_xdy, r_dst_LI_mxydx, r_dst_LI_xydy)
    
!    IF (par%master) THEN
!      WRITE(0,*) ' Checksum: r_dst_nV     = ', SUM(r_dst_nV    ), ' (should be 16694)'
!      WRITE(0,*) ' Checksum: r_dst_vi_src = ', SUM(r_dst_vi_src), ' (should be 35005259)'
!      WRITE(0,*) ' Checksum: r_dst_LI_xdy = ', SUM(r_dst_LI_xdy), ' (should be ~zero)'
!    END IF
!    CALL sync

    ! Integrate around domain boundary
    
!    IF (par%master) WRITE(0,*) '  Remapping: integrate around domain boundary'
    
    IF (par%master) CALL IntegrateAroundDomainBoundary( mesh_dst, mesh_src, Vor_lines_dst, Vor_lines_src, &
      r_dst_nV, r_dst_vi_src, r_dst_LI_xdy, r_dst_LI_mxydx, r_dst_LI_xydy)
    CALL sync
...
!      WRITE(0,*) ' Checksum: r_dst_nV     = ', SUM(r_dst_nV    ), ' (should be 16694)'
!      WRITE(0,*) ' Checksum: r_dst_vi_src = ', SUM(r_dst_vi_src), ' (should be 35005259)'
!      WRITE(0,*) ' Checksum: r_dst_LI_xdy = ', SUM(r_dst_LI_xdy), ' (should be 2.2200e+12)'
!    END IF
!    CALL sync

    ! Add contributions from mesh_src to mesh_dst
    
!    IF (par%master) WRITE(0,*) '  Remapping: add contributions from mesh_src to mesh_dst'
    
    CALL AddContributionsFromOppositeMesh( mesh_dst, r_dst_nV, r_dst_vi_src, r_dst_LI_xdy, r_dst_LI_mxydx, r_dst_LI_xydy, &
      r_src_nV, r_src_vi_dst, r_src_LI_xdy, r_src_LI_mxydx, r_src_LI_xydy)
    
!    IF (par%master) THEN
!      WRITE(0,*) ' Checksum: r_dst_LI_xdy = ', SUM(r_dst_LI_xdy), ' (should be 2203572811634.864)'
!    END IF
!    CALL sync

    ! Finish incomplete mesh_dst vertices
    
!    IF (par%master) WRITE(0,*) '  Remapping: finish incomplete mesh_dst vertices'
  
    CALL FinishIncompleteVertices( mesh_dst, mesh_src, nV_max, Vor_lines_dst, r_dst_nV, r_dst_vi_src, r_dst_LI_xdy, r_dst_LI_mxydx, r_dst_LI_xydy, &
      r_src_nV, r_src_vi_dst, r_src_LI_xdy, r_src_LI_mxydx, r_src_LI_xydy)
    
!    IF (par%master) THEN
!      WRITE(0,*) ' Checksum: r_dst_nV     = ', SUM(r_dst_nV    ), ' (should be 16752)'
!      WRITE(0,*) ' Checksum: r_dst_vi_src = ', SUM(r_dst_vi_src), ' (should be 35137018)'
!      WRITE(0,*) ' Checksum: r_dst_LI_xdy = ', SUM(r_dst_LI_xdy), ' (should be 2.2200e+12)'
!    END IF
!    CALL sync
    ...
    CALL CalculateRemappingWeightsFromLineIntegrals( mesh_dst, mesh_src, r_dst_nV, r_dst_vi_src, r_dst_LI_xdy, r_dst_LI_mxydx, r_dst_LI_xydy, map)
      
    ! Check if everything worked
    
    CALL CheckIfRemappingIsConservative( mesh_src, mesh_dst, map)
    
    ! Clean up after yourself 
       
    CALL CreateRemappingArrays_conservative_deallocate_memory( &
      proc_domain_V_src, proc_domain_Ac_src, proc_domain_V_dst, proc_domain_Ac_dst, &
      wVor_lines_src, wVor_lines_dst, wVor_vi_ti_src, wVor_vi_ti_dst, &
      wr_Ac_src_nV, wr_Ac_src_vi_dst_left, wr_Ac_src_vi_dst_right, wr_Ac_src_LI_xdy, wr_Ac_src_LI_mxydx, wr_Ac_src_LI_xydy, &
      wr_Ac_dst_nV, wr_Ac_dst_vi_src_left, wr_Ac_dst_vi_src_right, wr_Ac_dst_LI_xdy, wr_Ac_dst_LI_mxydx, wr_Ac_dst_LI_xydy, &
  ...
  END SUBROUTINE CreateRemappingArrays_conservative  
  SUBROUTINE CreateRemappingArrays_conservative_allocate_memory( mesh_src, mesh_dst, nV_max, &
    proc_domain_V_src, proc_domain_Ac_src, proc_domain_V_dst, proc_domain_Ac_dst, &
     Vor_lines_src,  Vor_lines_dst,  Vor_vi_ti_src,  Vor_vi_ti_dst, &
    wVor_lines_src, wVor_lines_dst, wVor_vi_ti_src, wVor_vi_ti_dst, &
     r_Ac_src_nV,  r_Ac_src_vi_dst_left,  r_Ac_src_vi_dst_right,  r_Ac_src_LI_xdy,  r_Ac_src_LI_mxydx,  r_Ac_src_LI_xydy, &
    ...
    INTEGER,  DIMENSION(:    ), ALLOCATABLE, INTENT(INOUT) :: proc_domain_V_src, proc_domain_Ac_src, proc_domain_V_dst, proc_domain_Ac_dst
    ...
    ! Process domain maps
    ALLOCATE( proc_domain_V_src(  mesh_src%nV))
    ALLOCATE( proc_domain_Ac_src( mesh_src%nAc))
    ALLOCATE( proc_domain_V_dst(  mesh_dst%nV))
    ...
    CALL allocate_shared_memory_dp_3D(  mesh_src%nAc, 2, 2, Vor_lines_src, wVor_lines_src)
    CALL allocate_shared_memory_dp_3D(  mesh_dst%nAc, 2, 2, Vor_lines_dst, wVor_lines_dst)
    CALL allocate_shared_memory_int_2D( mesh_src%nAc, 6,    Vor_vi_ti_src, wVor_vi_ti_src)
    CALL allocate_shared_memory_int_2D( mesh_dst%nAc, 6,    Vor_vi_ti_dst, wVor_vi_ti_dst)
    
    ! Results from integrating over all Voronoi cell boundary lines - Aci data
    
    CALL allocate_shared_memory_int_1D( mesh_src%nAc,          r_Ac_src_nV,            wr_Ac_src_nV           )
    CALL allocate_shared_memory_int_2D( mesh_src%nAc, nV_max,  r_Ac_src_vi_dst_left,   wr_Ac_src_vi_dst_left  )
    CALL allocate_shared_memory_int_2D( mesh_src%nAc, nV_max,  r_Ac_src_vi_dst_right,  wr_Ac_src_vi_dst_right )
    CALL allocate_shared_memory_dp_2D(  mesh_src%nAc, nV_max,  r_Ac_src_LI_xdy,        wr_Ac_src_LI_xdy       )
    CALL allocate_shared_memory_dp_2D(  mesh_src%nAc, nV_max,  r_Ac_src_LI_mxydx,      wr_Ac_src_LI_mxydx     )
    CALL allocate_shared_memory_dp_2D(  mesh_src%nAc, nV_max,  r_Ac_src_LI_xydy,       wr_Ac_src_LI_xydy      )
    
    CALL allocate_shared_memory_int_1D( mesh_dst%nAc,          r_Ac_dst_nV,            wr_Ac_dst_nV           )
    CALL allocate_shared_memory_int_2D( mesh_dst%nAc, nV_max,  r_Ac_dst_vi_src_left,   wr_Ac_dst_vi_src_left  )
    CALL allocate_shared_memory_int_2D( mesh_dst%nAc, nV_max,  r_Ac_dst_vi_src_right,  wr_Ac_dst_vi_src_right )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nAc, nV_max,  r_Ac_dst_LI_xdy,        wr_Ac_dst_LI_xdy       )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nAc, nV_max,  r_Ac_dst_LI_mxydx,      wr_Ac_dst_LI_mxydx     )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nAc, nV_max,  r_Ac_dst_LI_xydy,       wr_Ac_dst_LI_xydy      )

    ! Results from integrating over all Voronoi cell boundary lines - vertex data
    
    CALL allocate_shared_memory_int_1D( mesh_src%nV,           r_src_nV,               wr_src_nV              )
    CALL allocate_shared_memory_int_2D( mesh_src%nV,  nV_max,  r_src_vi_dst,           wr_src_vi_dst          )
    CALL allocate_shared_memory_dp_2D(  mesh_src%nV,  nV_max,  r_src_LI_xdy,           wr_src_LI_xdy          )
    CALL allocate_shared_memory_dp_2D(  mesh_src%nV,  nV_max,  r_src_LI_mxydx,         wr_src_LI_mxydx        )
    CALL allocate_shared_memory_dp_2D(  mesh_src%nV,  nV_max,  r_src_LI_xydy,          wr_src_LI_xydy         )
     
    CALL allocate_shared_memory_int_1D( mesh_dst%nV,           r_dst_nV,               wr_dst_nV              )
    CALL allocate_shared_memory_int_2D( mesh_dst%nV,  nV_max,  r_dst_vi_src,           wr_dst_vi_src          )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nV,  nV_max,  r_dst_LI_xdy,           wr_dst_LI_xdy          )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nV,  nV_max,  r_dst_LI_mxydx,         wr_dst_LI_mxydx        )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nV,  nV_max,  r_dst_LI_xydy,          wr_dst_LI_xydy         )
    
    ! Actual mapping weights
     
    CALL allocate_shared_memory_int_1D( mesh_dst%nV,           map%nV,                 map%wnV                )
    CALL allocate_shared_memory_int_2D( mesh_dst%nV,  nV_max,  map%vi,                 map%wvi                )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nV,  nV_max,  map%w0,                 map%ww0                )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nV,  nV_max,  map%w1x,                map%ww1x               )
    CALL allocate_shared_memory_dp_2D(  mesh_dst%nV,  nV_max,  map%w1y,                map%ww1y               )
    
  END SUBROUTINE CreateRemappingArrays_conservative_allocate_memory
  SUBROUTINE FindVoronoiBoundaryLines( mesh, Vor_lines, Vor_vi_ti)
    ...
    DO aci = mesh%c1, mesh%c2
      ...
      IF (.NOT. IsBoundarySegment( mesh, via, vib)) THEN
        ...
        IF     (ccl(1) < mesh%xmin) THEN
          ...
          CALL LineFromPoints( ccr, ccl, l1a, l1b, l1c)
          CALL LineFromPoints( p1,  p2 , l2a, l2b, l2c)
          CALL LineLineIntersection( l1a, l1b, l1c, l2a, l2b, l2c, ccl)
        ...
        IF     (ccr(1) < mesh%xmin) THEN
          ...
          CALL LineFromPoints( ccr, ccl, l1a, l1b, l1c)
          CALL LineFromPoints( p1,  p2 , l2a, l2b, l2c)
          CALL LineLineIntersection( l1a, l1b, l1c, l2a, l2b, l2c, ccr)
      ...
      ELSE ! IF (.NOT. IsBoundarySegment( mesh, via, vib)) THEN
      ...
      END IF ! IF (.NOT. IsBoundarySegment( mesh, via, vib)) THEN
      
    END DO ! DO aci = mesh%c1, mesh%c2
    CALL sync

  END SUBROUTINE FindVoronoiBoundaryLines
  SUBROUTINE DetermineProcessDomains( mesh, proc_domain_V, proc_domain_Ac, vi_start_proc)
    ...
    DO i = 0, par%n-1
      ...
      orientation = 0
      CALL DetermineProcessXYDomain_balanced( mesh, i, orientation, xmin, xmax, ymin, ymax)
  ...
  END SUBROUTINE DetermineProcessDomains
  SUBROUTINE IntegrateOverVoronoiBoundaries( mesh_top, mesh_bot, proc_domain_V_top, proc_domain_Ac_top, &
    ...
    INTEGER,  DIMENSION(:    ),              INTENT(IN)    :: proc_domain_V_top
   ...
   !     IF (proc_domain_V_top( vi_top) /= par%i) CYCLE ! This neighbour is outside of this process domain
      ...
      DO ci = 1, mesh_top%nC( vi_top)
        ...
        ! Integrate
        CALL CalculateLineIntegralContributions( mesh_bot, Vor_lines_top, Vor_lines_bot, Vor_vi_ti_bot, aci, &
  ...
  END SUBROUTINE IntegrateOverVoronoiBoundaries
  SUBROUTINE RearrangeContributionsFromLinesToVertices( mesh, r_Ac_nV, r_Ac_vi_bot_left, r_Ac_vi_bot_right, &
  ...
  END SUBROUTINE RearrangeContributionsFromLinesToVertices
  SUBROUTINE IntegrateAroundDomainBoundary( mesh_top, mesh_bot, Vor_lines_top, Vor_lines_bot, r_dst_nV, r_dst_vi_src, r_dst_LI_xdy, r_dst_LI_mxydx, r_dst_LI_xydy)
  ...
  ! ==================
    ...
    DO WHILE (.NOT.Finished)
      ...
      LI_xdy   = LineIntegral_xdy(   p1, p, mesh_top%tol_dist)
      LI_mxydx = LineIntegral_mxydx( p1, p, mesh_top%tol_dist)
      LI_xydy  = LineIntegral_xydy(  p1, p, mesh_top%tol_dist)
  ...
  END SUBROUTINE IntegrateAroundDomainBoundary
  SUBROUTINE AddContributionsFromOppositeMesh( mesh_top, r_top_nV, r_top_vi_bot, r_top_LI_xdy, r_top_LI_mxydx, r_top_LI_xydy, &
  ...
  END SUBROUTINE AddContributionsFromOppositeMesh
  SUBROUTINE FinishIncompleteVertices( mesh_top, mesh_bot, nV_max, Vor_lines_top, r_top_nV, r_top_vi_bot, r_top_LI_xdy, r_top_LI_mxydx, r_top_LI_xydy, &
    ...
    DO vi_top = mesh_top%v1, mesh_top%v2
      ...
      IF (Eint > 1e-6_dp) THEN
        ...
        vi_bot = 5
        p = mesh_top%V( vi_top,:)
        CALL FindContainingVertex( mesh_bot, p, vi_bot)
        ...
        DO WHILE (mesh_bot%VStackN1 > 0)
          ...
          q = mesh_bot%V( vi_bot,:)
          IF (IsInVoronoiCell_remap( mesh_top, vi_top, Vor_lines_top, q)) THEN
          ...
          END IF ! IF (IsInVoronoiCell_remap( mesh_top, vi_top, Vor_lines_top, q)) THEN
  ...
  END SUBROUTINE FinishIncompleteVertices
  SUBROUTINE CalculateRemappingWeightsFromLineIntegrals( mesh_top, mesh_bot, r_top_nV, r_top_vi, r_top_LI_xdy, r_top_LI_mxydx, r_top_LI_xydy, map)
  ...
  END SUBROUTINE CalculateRemappingWeightsFromLineIntegrals
  SUBROUTINE CheckIfRemappingIsConservative( mesh_src, mesh_dst, map)
    ...
    CALL allocate_shared_memory_dp_1D( mesh_src%nV, d_src, wd_src)
    CALL allocate_shared_memory_dp_1D( mesh_dst%nV, d_dst, wd_dst)
    
    d_src(mesh_src%v1:mesh_src%v2) = 1._dp
    CALL sync
    CALL RemapMesh2Mesh_cons_2nd_order_2D( mesh_src, mesh_dst, map, d_src, d_dst)
    CALL sync
...
!    CALL sync     
    ...
    CALL deallocate_shared_memory( wd_src)
    CALL deallocate_shared_memory( wd_dst)    
    NULLIFY( d_src)
    NULLIFY( d_dst)
    
  END SUBROUTINE CheckIfRemappingIsConservative
  SUBROUTINE CreateRemappingArrays_conservative_deallocate_memory( &
    proc_domain_V_src, proc_domain_Ac_src, proc_domain_V_dst, proc_domain_Ac_dst, &
    wVor_lines_src, wVor_lines_dst, wVor_vi_ti_src, wVor_vi_ti_dst, &
    wr_Ac_src_nV, wr_Ac_src_vi_dst_left, wr_Ac_src_vi_dst_right, wr_Ac_src_LI_xdy, wr_Ac_src_LI_mxydx, wr_Ac_src_LI_xydy, &
    wr_Ac_dst_nV, wr_Ac_dst_vi_src_left, wr_Ac_dst_vi_src_right, wr_Ac_dst_LI_xdy, wr_Ac_dst_LI_mxydx, wr_Ac_dst_LI_xydy, &
    ...
    DEALLOCATE( proc_domain_V_src)
    DEALLOCATE( proc_domain_V_dst)
    DEALLOCATE( proc_domain_Ac_src)
    DEALLOCATE( proc_domain_Ac_dst)   
            
    CALL deallocate_shared_memory( wVor_lines_src)
    CALL deallocate_shared_memory( wVor_lines_dst)
    CALL deallocate_shared_memory( wVor_vi_ti_src)
    CALL deallocate_shared_memory( wVor_vi_ti_dst) 
    
    CALL deallocate_shared_memory( wr_Ac_src_nV          )
    CALL deallocate_shared_memory( wr_Ac_src_vi_dst_left )
    CALL deallocate_shared_memory( wr_Ac_src_vi_dst_right)
    CALL deallocate_shared_memory( wr_Ac_src_LI_xdy      )
    CALL deallocate_shared_memory( wr_Ac_src_LI_mxydx    )
    CALL deallocate_shared_memory( wr_Ac_src_LI_xydy     )
    
    CALL deallocate_shared_memory( wr_Ac_dst_nV          )
    CALL deallocate_shared_memory( wr_Ac_dst_vi_src_left )
    CALL deallocate_shared_memory( wr_Ac_dst_vi_src_right)
    CALL deallocate_shared_memory( wr_Ac_dst_LI_xdy      )
    CALL deallocate_shared_memory( wr_Ac_dst_LI_mxydx    )
    CALL deallocate_shared_memory( wr_Ac_dst_LI_xydy     )
    
    CALL deallocate_shared_memory( wr_src_nV      )
    CALL deallocate_shared_memory( wr_src_vi_dst  )
    CALL deallocate_shared_memory( wr_src_LI_xdy  )
    CALL deallocate_shared_memory( wr_src_LI_mxydx)
    CALL deallocate_shared_memory( wr_src_LI_xydy )
     
    CALL deallocate_shared_memory( wr_dst_nV      )
    CALL deallocate_shared_memory( wr_dst_vi_src  )
    CALL deallocate_shared_memory( wr_dst_LI_xdy  )
    CALL deallocate_shared_memory( wr_dst_LI_mxydx)
    CALL deallocate_shared_memory( wr_dst_LI_xydy ) 
    
  END SUBROUTINE CreateRemappingArrays_conservative_deallocate_memory
  
  SUBROUTINE CalculateLineIntegralContributions( mesh_bot, Vor_lines_top, Vor_lines_bot, Vor_vi_ti_bot, aci, &
          r_sng_nV, r_sng_vi_left, r_sng_vi_right, r_sng_LI_xdy, r_sng_LI_mxydx, r_sng_LI_xydy, vi_bot, CountCoincidences, nV_max)
  ...
  !  WRITE(0,*) '  Process ', par%i, ' - Integrating over the Voronoi boundary between vertices ', mesh_top%Aci(aci,1), ' and ', mesh_top%Aci(aci,2), ' (aci = ', aci, ')'
    ...
    CALL TraceLineThroughMesh_start( mesh_bot, p, vi_bot, Vor_lines_bot, aci_coincide, tri_coincide, vi_in)
    ...
    DO WHILE (.NOT. Finished)
      ...
      CALL TraceLineThroughMesh( mesh_bot, p, q, Vor_lines_bot, Vor_vi_ti_bot, aci_coincide, tri_coincide, vi_in, p_next, vi_bot_left, vi_bot_right, Coincides, Finished)

      ! Integrate over the line section p-p_next
      DoAdd = .TRUE.
      IF (Coincides .AND. (.NOT. CountCoincidences)) DoAdd = .FALSE.
      IF (DoAdd) THEN
        CALL AddLineIntegralContributions( p, p_next, vi_bot_left, vi_bot_right, r_sng_nV, r_sng_vi_left, r_sng_vi_right, r_sng_LI_xdy, r_sng_LI_mxydx, r_sng_LI_xydy, mesh_bot%tol_dist)
      END IF
  ...
  END SUBROUTINE CalculateLineIntegralContributions  
  SUBROUTINE AddLineIntegralContributions( p, q, vi_bot_left, vi_bot_right, r_sng_nV, r_sng_vi_left, r_sng_vi_right, r_sng_LI_xdy, r_sng_LI_mxydx, r_sng_LI_xydy, tol)
    ...
    r_sng_LI_xdy(   nV) = LineIntegral_xdy(   p, q, tol)
    r_sng_LI_mxydx( nV) = LineIntegral_mxydx( p, q, tol)
    r_sng_LI_xydy(  nV) = LineIntegral_xydy(  p, q, tol)
    
  END SUBROUTINE AddLineIntegralContributions
  
  SUBROUTINE TraceLineThroughMesh_start( mesh_bot, p, vi_bot, Vor_lines_bot, aci_coincide, tri_coincide, vi_in)
    ...
    DO ci = 1, mesh_bot%nC( vi_bot)
      ...
      IF (LiesOnLineSegment( pa, pb, p, mesh_bot%tol_dist) .OR. NORM2(p-pa) < mesh_bot%tol_dist .OR. NORM2(p-pb) < mesh_bot%tol_dist) THEN
  ...
  END SUBROUTINE TraceLineThroughMesh_start
  SUBROUTINE TraceLineThroughMesh( mesh_bot, p, q, Vor_lines_bot, Vor_vi_ti_bot, aci_coincide, tri_coincide, vi_in, p_next, vi_bot_left, vi_bot_right, Coincides, Finished)
    ...
    IF (tri_coincide > 0) THEN
      ...
      CALL TraceLineThroughMesh_tri_coincide( mesh_bot, p, q, Vor_lines_bot, Vor_vi_ti_bot, aci_coincide, tri_coincide, vi_in, p_next, vi_bot_left, vi_bot_right, Coincides, Finished)
    ELSEIF (aci_coincide > 0) THEN
      ...
      CALL TraceLineThroughMesh_aci_coincide( mesh_bot, p, q, Vor_lines_bot, Vor_vi_ti_bot, aci_coincide, tri_coincide, vi_in, p_next, vi_bot_left, vi_bot_right, Coincides, Finished)
    ELSE
      ! p lies inside the interior of mesh_bot vertex vi_in
      CALL TraceLineThroughMesh_vi_interior(  mesh_bot, p, q, Vor_lines_bot, Vor_vi_ti_bot, aci_coincide, tri_coincide, vi_in, p_next, vi_bot_left, vi_bot_right, Coincides, Finished)
    END IF
    
  END SUBROUTINE TraceLineThroughMesh
  SUBROUTINE TraceLineThroughMesh_aci_coincide( mesh_bot, p, q, Vor_lines_bot, Vor_vi_ti_bot, aci_coincide, tri_coincide, vi_in, p_next, vi_bot_left, vi_bot_right, Coincides, Finished)
    ! Given a point p that either:
    ! - coincides with mesh_bot Voronoi boundary line aci_coincide,
    ! - coincides with mesh_bot triangle circumcenter (= Voronoi boundary vertex) tri_coincide, or
    ! - lies in the interior of Voronoi cell vi_in,
    ...
    ! Safety
    IF (.NOT. LiesOnLineSegment( ccr, ccl, p, mesh_bot%tol_dist)) THEN
      WRITE(0,*) '    Remapping - ERROR: coincidence indicators dont match!'
      STOP
    END IF
  ...
  ! ================================================
    ...
    IF     (LiesOnLineSegment( p, ccr, q, mesh_bot%tol_dist) .AND. NORM2(p-ccr) > mesh_bot%tol_dist) THEN
      ...
      Finished     = .TRUE.
      RETURN
    ELSEIF (LiesOnLineSegment( p, ccl, q, mesh_bot%tol_dist) .AND. NORM2(p-ccl) > mesh_bot%tol_dist) THEN
  ...
  ! ==========================================================
    ...
    DO ci = 1, mesh_bot%nC( via)
      ...
      IF (NORM2(q-cc1) < mesh_bot%tol_dist .OR. NORM2(q-cc2) < mesh_bot%tol_dist .OR. LiesOnLineSegment( cc1, cc2, q, mesh_bot%tol_dist)) THEN
  ...
  ! ===========================================================================
    ...
    IF     (IsInVoronoiCell_remap( mesh_bot, via, Vor_lines_bot, q)) THEN
      ...
      Finished     = .TRUE.
      RETURN  
    ELSEIF (IsInVoronoiCell_remap( mesh_bot, vib, Vor_lines_bot, q)) THEN
  ...
  ! =======================================================================
    ...
    IF     (LiesOnLineSegment( p, q, ccr, mesh_bot%tol_dist) .AND. NORM2(p-ccr) > mesh_bot%tol_dist) THEN
      ...
      Coincides    = .TRUE.
      RETURN
    ELSEIF (LiesOnLineSegment( p, q, ccl, mesh_bot%tol_dist) .AND. NORM2(p-ccl) > mesh_bot%tol_dist) THEN
  ...
  ! ==================================================================
    ...
    DO ci = 1, mesh_bot%nC( via)
      ...
      IF     (LiesOnLineSegment( p, q, cc1, mesh_bot%tol_dist)) THEN
        ...
        tri_coincide = ti1
        RETURN
      ELSEIF (LiesOnLineSegment( p, q, cc2, mesh_bot%tol_dist)) THEN
        ! pq passes through this Voronoi vertex
        p_next       = cc2
        vi_bot_left  = via
        vi_bot_right = via
      ...
      CALL SegmentIntersection( cc1, cc2, p, q, llis, do_cross, mesh_bot%tol_dist)
      IF (do_cross) THEN
        ! pq cross this line
        p_next       = llis
        vi_bot_left  = via
  ...
  ! ==================================================================
    ...
    WRITE(0,*) '     TraceLineThroughMesh_aci_coincide - ERROR: reached the unreachable point!'
    STOP
    
  END SUBROUTINE TraceLineThroughMesh_aci_coincide
  SUBROUTINE TraceLineThroughMesh_tri_coincide( mesh_bot, p, q, Vor_lines_bot, Vor_vi_ti_bot, aci_coincide, tri_coincide, vi_in, p_next, vi_bot_left, vi_bot_right, Coincides, Finished)
    ! Given a point p that either:
    ! - coincides with mesh_bot Voronoi boundary line aci_coincide,
    ! - coincides with mesh_bot triangle circumcenter (= Voronoi boundary vertex) tri_coincide, or
    ! - lies in the interior of Voronoi cell vi_in,
    ...
    DO ci = 1, mesh_bot%VStackN1
      ...
      IF     (NORM2(p-ccl) < mesh_bot%tol_dist .AND. LiesOnLineSegment( p, q, ccr, mesh_bot%tol_dist)) THEN
        ! q coincides with ccr
        p_next       = ccr
        vi_bot_left  = vib
        vi_bot_right = via
        ...
        tri_coincide = tir
        RETURN
      ELSEIF (NORM2(p-ccr) < mesh_bot%tol_dist .AND. LiesOnLineSegment( p, q, ccl, mesh_bot%tol_dist)) THEN
        ! q coincides with ccl
        p_next       = ccl
        vi_bot_left  = via
        vi_bot_right = vib
    ...
    DO vvi = 1, mesh_bot%VStackN2
      ...
      DO ci = 1, mesh_bot%nC( vi)
        ...
        IF (NORM2(q-ccr) < mesh_bot%tol_dist .OR. NORM2(q-ccl) < mesh_bot%tol_dist .OR. LiesOnLineSegment( ccl, ccr, q, mesh_bot%tol_dist)) THEN
      ...
      IF (IsInVoronoiCell_remap( mesh_bot, vi, Vor_lines_bot, q)) THEN
        ! q lies on the boundary of this Voronoi cell
        p_next       = q
        vi_bot_left  = vi
        vi_bot_right = vi
    ...
    DO vvi = 1, mesh_bot%VStackN2
      ...
      DO ci = 1, mesh_bot%nC( vi)
        ...
        CALL SegmentIntersection( p, q, ccl, ccr, llis, do_cross, mesh_bot%tol_dist)
    ...
    ! None of the available options are .TRUE. this shouldn't be possible
    WRITE(0,*) '     TraceLineThroughMesh_tri_coincide - ERROR: reached the unreachable point!'
    STOP
    
  END SUBROUTINE TraceLineThroughMesh_tri_coincide
  SUBROUTINE TraceLineThroughMesh_vi_interior( mesh_bot, p, q, Vor_lines_bot, Vor_vi_ti_bot, aci_coincide, tri_coincide, vi_in, p_next, vi_bot_left, vi_bot_right, Coincides, Finished)
    ! Given a point p that either:
    ! - coincides with mesh_bot Voronoi boundary line aci_coincide,
    ! - coincides with mesh_bot triangle circumcenter (= Voronoi boundary vertex) tri_coincide, or
    ! - lies in the interior of Voronoi cell vi_in,
    ...
    ! Safety
    via = vi_in
    CALL FindContainingVertex( mesh_bot, p, via)
    IF (vi_in /= via) THEN
      WRITE(0,*) '    Remapping - ERROR: coincidence indicators dont match!'
      STOP
    END IF
    ...
    DO ci = 1, mesh_bot%nC( via)
      ...
      ccl = [Vor_lines_bot( aci,2,1), Vor_lines_bot( aci,2,2)]
      IF (NORM2(ccr-ccl) < mesh_bot%tol_dist) CYCLE
      IF (NORM2(q-ccl) < mesh_bot%tol_dist .OR. NORM2(q-ccr) < mesh_bot%tol_dist .OR. LiesOnLineSegment( ccr, ccl, q, mesh_bot%tol_dist)) THEN
    ...
    DO ci = 1, mesh_bot%nC( via)
      ...
      IF     (LiesOnLineSegment(p, q, ccr, mesh_bot%tol_dist)) THEN
        ! q passes through this circumcenter
        p_next       = ccr
        tri_coincide = tir
        RETURN
      ELSEIF (LiesOnLineSegment(p, q, ccl, mesh_bot%tol_dist)) THEN
        ! q passes through this circumcenter
        p_next       = ccl
        tri_coincide = til
        RETURN
      ...
      CALL SegmentIntersection( ccr, ccl, p, q, llis, do_cross, mesh_bot%tol_dist)
      IF (do_cross) THEN
        ! pq cross this line
        p_next       = llis
        aci_coincide = aci
    ...
    ! None of the available options are .TRUE. this shouldn't be possible
    WRITE(0,*) '     TraceLineThroughMesh_vi_interior - ERROR: reached the unreachable point!'
    STOP
    
  END SUBROUTINE TraceLineThroughMesh_vi_interior
    
  FUNCTION IsInVoronoiCell_remap( mesh, vi, Vor_lines, q) RESULT(isso)
  ...
  END FUNCTION IsInVoronoiCell_remap
  
  SUBROUTINE RemapMesh2Mesh_trilin_2D( mesh_dst, map, d_src, d_dst)
  ...
  END SUBROUTINE RemapMesh2mesh_trilin_2D
  SUBROUTINE RemapMesh2Mesh_trilin_3D( mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using trilinear interpolation
  ...
  END SUBROUTINE RemapMesh2mesh_trilin_3D
  SUBROUTINE RemapMesh2Mesh_trilin_monthly( mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using trilinear interpolation
  ...
  END SUBROUTINE RemapMesh2mesh_trilin_monthly
  SUBROUTINE RemapMesh2Mesh_nearest_neighbour_2D( mesh_dst, map, d_src, d_dst)
  ...
  END SUBROUTINE RemapMesh2Mesh_nearest_neighbour_2D
  SUBROUTINE RemapMesh2Mesh_nearest_neighbour_3D( mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using nearest neighbour interpolation
  ...
  END SUBROUTINE RemapMesh2Mesh_nearest_neighbour_3D
  SUBROUTINE RemapMesh2Mesh_cons_1st_order_2D( mesh_dst, map, d_src, d_dst)
  ...
  END SUBROUTINE RemapMesh2Mesh_cons_1st_order_2D
  SUBROUTINE RemapMesh2Mesh_cons_1st_order_3D( mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using 1st order conservative remapping
  ...
  END SUBROUTINE RemapMesh2Mesh_cons_1st_order_3D
  SUBROUTINE RemapMesh2Mesh_cons_2nd_order_2D( mesh_src, mesh_dst, map, d_src, d_dst)
    ...
    CALL allocate_shared_memory_dp_1D(  mesh_src%nV, ddx_src, wddx_src)
    CALL allocate_shared_memory_dp_1D(  mesh_src%nV, ddy_src, wddy_src)
    
    d_dst(mesh_dst%v1:mesh_dst%v2) = 0._dp
    CALL sync
    
    CALL GetMeshDerivatives( mesh_src, d_src, ddx_src, ddy_src)
    CALL sync
    ...
    CALL deallocate_shared_memory( wddx_src)
    CALL deallocate_shared_memory( wddy_src)
    
    NULLIFY( ddx_src)
    NULLIFY( ddy_src)
    
  END SUBROUTINE RemapMesh2Mesh_cons_2nd_order_2D
  SUBROUTINE RemapMesh2Mesh_cons_2nd_order_2D_monthly( mesh_src, mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using 2nd order conservative remapping
    ...
    CALL allocate_shared_memory_dp_2D(  mesh_src%nV, 12, ddx_src, wddx_src)
    CALL allocate_shared_memory_dp_2D(  mesh_src%nV, 12, ddy_src, wddy_src)
    
    d_dst(mesh_dst%v1:mesh_dst%v2,:) = 0._dp
    
    CALL GetMeshDerivatives_3D( mesh_src, d_src, ddx_src, ddy_src, 12)
  ...
  END SUBROUTINE RemapMesh2Mesh_cons_2nd_order_2D_monthly
  SUBROUTINE RemapMesh2Mesh_cons_2nd_order_3D( mesh_src, mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using 2nd order conservative remapping
    ...
    CALL allocate_shared_memory_dp_2D(  mesh_src%nV, C%nz, ddx_src, wddx_src)
    CALL allocate_shared_memory_dp_2D(  mesh_src%nV, C%nz, ddy_src, wddy_src)
    
    d_dst(mesh_dst%v1:mesh_dst%v2,:) = 0._dp
    
    CALL GetMeshDerivatives_3D( mesh_src, d_src, ddx_src, ddy_src, C%nz)
  ...
  END SUBROUTINE RemapMesh2Mesh_cons_2nd_order_3D
