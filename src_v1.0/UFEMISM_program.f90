...
!   "MapDataFromMeshToMesh" routines contained in the "UFEMISM_main_model" module.
  ...
  CHARACTER(LEN=256), PARAMETER          :: version_number = '1.0'
  
  INTEGER                                :: p, iargc, ierr, status(MPI_STATUS_SIZE)
  ...
  IF (par%master) THEN
    ...
    ! Send config filename to slave processes
    DO p = 1, par%n-1
      CALL MPI_SEND( config_filename, 256, MPI_CHAR, p, 0, MPI_COMM_WORLD, ierr)
    END DO
  ELSE 
    ! Receive config filename from master process
    CALL MPI_RECV( config_filename, 256, MPI_CHAR, 0, MPI_ANY_TAG, MPI_COMM_WORLD, status, ierr) 
  END IF
  ...
  END IF 
  IF (par%master) THEN
    ! Send output directory name to slave processes
    DO p = 1, par%n-1
      CALL MPI_SEND( C%output_dir, 256, MPI_CHAR, p, 0, MPI_COMM_WORLD, ierr)
    END DO
  ELSE 
    ! Receive output directory name master process
    CALL MPI_RECV( C%output_dir, 256, MPI_CHAR, 0, MPI_ANY_TAG, MPI_COMM_WORLD, status, ierr)
  ...
  IF (C%which_icesheets(1:1) == 'N') CALL initialise_model(NAM, 'NAM', matrix)
  IF (C%which_icesheets(2:2) == 'E') CALL initialise_model(EAS, 'EAS', matrix)
  IF (C%which_icesheets(3:3) == 'G') CALL initialise_model(GRL, 'GRL', matrix)
  IF (C%which_icesheets(4:4) == 'A') CALL initialise_model(ANT, 'ANT', matrix)
  ...
  DO WHILE (t_coupling < C%end_time_of_run)
    ...
    IF (C%which_icesheets(1:1) == 'N') CALL run_model(NAM, matrix, t_end_models)
    IF (C%which_icesheets(2:2) == 'E') CALL run_model(EAS, matrix, t_end_models)
    IF (C%which_icesheets(3:3) == 'G') CALL run_model(GRL, matrix, t_end_models)
    IF (C%which_icesheets(4:4) == 'A') CALL run_model(ANT, matrix, t_end_models)
    ...
    GMSL_GRL = 0._dp
    GMSL_ANT = 0._dp
    IF (C%which_icesheets(1:1) == 'N') GMSL_NAM = NAM%GMSL_contribution
    IF (C%which_icesheets(1:1) == 'E') GMSL_EAS = EAS%GMSL_contribution
    IF (C%which_icesheets(1:1) == 'G') GMSL_GRL = GRL%GMSL_contribution
    IF (C%which_icesheets(1:1) == 'A') GMSL_ANT = ANT%GMSL_contribution
