...
\title{UFEMISM v1.1 - documentation}
\date{2021-04-07}
...
\item \textbf{Download} the Github repository by clicking the green ''Code'' button, and selecting ''Download ZIP''. The file ''UFEMISM-master.zip'' should now be in your Downloads folder.
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{Fig_howto_03.png}
  \end{figure}
  
  \item \textbf{Run} the model by running the script ''run\textunderscore UFEMISM\textunderscore demo.csh''. This will start a short 10,000 year simulation (specifically, the first 10 kyr of the EISMINT\textunderscore 1 benchmark experiment) with a 16 km resolution. This should finish in about 20 seconds on a 2-core system.

  \begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Fig_howto_04.png}
  \end{figure}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Fig_howto_05.png}
  \end{figure}
  
  \item \textbf{Inspect} the output in the results folder ''results\textunderscore 20210407\textunderscore 001'' (with the current date in the name), which has been created by UFEMISM. The ''gridded'' output files (\textsc{restart\textunderscore grid\textunderscore ANT.nc} and \textsc{help\textunderscore fields\textunderscore grid\textunderscore ANT.nc}) should be compatible with any NetCDF viewing program (e.g. ncview). The ''mesh'' output files can be read with the different Matlab scripts in the ''MATLAB'' folder.
...
\item \textbf{Direct CO\textsubscript{2} / inverse-$\delta^{18}O$ forcing}
...
\begin{table}[H]
  \begin{center}
    \begin{tabular}{l|l|l|l|l|l|l}
      ...
      \hline
      Last glacial cycle & All & 120 kyr & 10 km & 3.8 Gb & 120 h & 7.5 h \\
    \end{tabular}
  \end{center}
\end{table}
...
\\
\textsc{debug} output can be written at any point in the code of the different ice-sheet model components. The \textsc{debug} structure has a number of fields where different types of data can be stored: integer and double precision types, and 2D, 3D, and 2D monthly dimensions, and on the Aa, Ac, or combined AaAc mesh (see the \textsc{data\textunderscore types\textunderscore module} for the full list). Using this in the code will look as follows:

\hrulefill
\begin{lstlisting}

debug%dp_2D_Aa_01( mesh%v1:mesh%v2) = ice%Hi( mesh%v1:mesh%v2)
debug%int_2D_Ac_04( mesh%ac1:mesh%ac2) = ice%mask_gl_Ac( mesh%ac1:mesh%ac2)
...
In order for this to have effect, the config option \textsc{do\textunderscore write\textunderscore debug\textunderscore data} must be turned on. Note that the debug file has no time dimension; whenever \textsc{write\textunderscore to\textunderscore debug\textunderscore file} is called, all data fields in the NetCDF file are overwritten. Note also that a new debug file is created after each mesh update, discarding the old one. Since the NetCDF file can only be created when a valid mesh exists, it cannot be used to track errors in the mesh creation routines.

\subsection{Computer resources}

\subsubsection{Computation time}

Shown in Fig. XX is the core time used by the different model components for a simulation of the last glacial cycle, where all four ice sheets were simulated simultaneously with a grounding-line resolution of 10 km. The model was run on 16 cores in parallel, so that the simulation took about 7.5 wall clock hours to finish. The lines show the computation requirements over the course of the simulation; the pie charts show the relative contribution from each model component averaged over the entire simulation.
...
As can be seen, the SIA dominates the computation time for most of the simulation. However, during the deglaciation large shelves form in North America and Eurasia, so that the SSA becomes a lot slower (although the SSA is always solved on all vertices, higher ice velocities generally mean more SOR iterations are required to reach a solution). Since the ice sheet geometry changes faster during the deglaciation, the mesh is also updated more frequently, which slightly  increases the computation times of both the SIA and the SSA.\\
\\
Berends et al. (2021) showed that total computation time scales with model resolution to about order 2.6. This means that the same simulation of the last glacial cycle with a ground-line resolution of 5 km can be expected to take about 45 wall clock hours to finish (about 750 core hours). For a resolution of 1 km, this number increases to 3,000 wall clock hours, over four months. This is not feasible in practise, until significant advances in the parallelisation of the model are made. Currently the model can run on only as many cores as can access the same physical memory chip (shared memory), typically 16, 24, or 32. When this limitation is removed, using dozens to possibly even hundreds of cores might be possible. Using 256 cores, wall clock time for a 1 km resolution simulation might be reduced to about 8 days - long, but definitely possible.
...
\begin{figure}[h!] \label{fig:resources_memory}
  \includegraphics[width=0.6\linewidth]{Fig_resources_memory.png}
