...
#include <petsc/finclude/petscksp.h>
  ...
  USE data_types_module,               ONLY: type_model_region, type_mesh, type_mesh_new, type_reference_geometry
  ...
  IMPLICIT NONE
  interface is_good_triangle
    procedure :: is_good_triangle
    procedure :: is_good_triangle_new
  end interface
  interface align_submeshes
    procedure :: align_submeshes
    procedure :: align_submeshes_new
  end interface
  interface is_good_triangle_geo_only
    procedure :: is_good_triangle_geo_only
    procedure :: is_good_triangle_geo_only_new
  end interface
  interface merge_submeshes
    procedure :: merge_submeshes
    procedure :: merge_submeshes_new
  end interface
  interface initialise_dummy_mesh
    procedure :: initialise_dummy_mesh
    procedure :: initialise_dummy_mesh_new
  end interface
  interface perturb_dummy_mesh
    procedure :: perturb_dummy_mesh
    procedure :: perturb_dummy_mesh_new
  end interface
  interface align_all_submeshes
    procedure :: align_all_submeshes
    procedure :: align_all_submeshes_new
  end interface
  interface refine_submesh_geo_only
    procedure :: refine_submesh_geo_only
    procedure :: refine_submesh_geo_only_new
  end interface
  interface merge_all_submeshes
    procedure :: merge_all_submeshes
  ...
  SUBROUTINE is_good_triangle_new( mesh, ti, refgeo_init, is_good)
  ...
  END SUBROUTINE is_good_triangle_new
  ...
  SUBROUTINE is_good_triangle_geo_only_new( mesh, ti, is_good)
    ! Check if triangle ti of the mesh is Good
    ! A triangle is not Good if:
    !   - its smallest internal angle is too small
    !   - its 2nd order surface deviation (=max(curvature)*typical_length) is too large
  ...
  END SUBROUTINE is_good_triangle_geo_only_new
  ...
  SUBROUTINE refine_submesh_geo_only_new( mesh)
    ! Refine a mesh. Single-core, but multiple submeshes can be done in parallel on different cores.
  ...
  END SUBROUTINE refine_submesh_geo_only_new
  ...
  SUBROUTINE Lloyds_algorithm_single_iteration_submesh_new( mesh)
    ! Lloyd's algorithm: move all vertices to the geometric centers of their Voronoi cells, and update the triangulation.
    ! This "smooths" the mesh, reducing resolution gradients and widening internal angles, thus making it more
    ! suitable for numerical methods (particularly the SSA).
  ...
  END SUBROUTINE Lloyds_algorithm_single_iteration_submesh_new
  
  ! == Align and merge submeshes created by parallel processes
  SUBROUTINE align_all_submeshes_new( submesh, orientation)
  ...
  END SUBROUTINE align_all_submeshes_new
  ...
  SUBROUTINE align_submeshes_new( submesh, orientation, p_left, p_right, nVl_east, nVr_west)
...
!    WRITE(0,'(A,I2,A)') '  align_submeshes - process ', par%i, ': finished'
  ...
  END SUBROUTINE align_submeshes_new
...
!    WRITE(0,'(A,I2,A)') '  align_submeshes - process ', par%i, ': finished'
  ...
  SUBROUTINE merge_all_submeshes_new( submesh, orientation)
    ! Iteratively merge the submeshes created by the different processes.
  ...
  END SUBROUTINE merge_all_submeshes_new
  SUBROUTINE merge_submeshes_new( submesh, mergelist, nmerge, orientation)
    ...
    TYPE(type_mesh_new)                               :: submesh_right
    INTEGER                                       :: status(MPI_STATUS_SIZE)
  ...
  END SUBROUTINE merge_submeshes_new
  ...
  SUBROUTINE initialise_dummy_mesh_new( mesh, xmin, xmax, ymin, ymax)
    ! Initialises a 5-vertex, 4-triangle "dummy"  mesh:
    !
    !   v4 - - - - - - - - v3   V          nC     C             niTri   iTri          edge_index
    !   | \              / |    -1 -1      3      2  5  4        2      1  4            6
  ...
  END SUBROUTINE initialise_dummy_mesh_new
  ...
  SUBROUTINE perturb_dummy_mesh_new( mesh, perturb_dir_global)
    ! "Perturb" the five-vertex dummy mesh; slightly offset the centre vertex,
    ! and split the four edges just next to their midpoints. This ensures that
    ! any new triangles created during mesh refinement are never cocircular.
  ...
  END SUBROUTINE perturb_dummy_mesh_new
