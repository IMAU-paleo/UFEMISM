MODULE mesh_memory_module
  ...
  SUBROUTINE AllocateMesh_primary(       mesh, region_name, nV_mem, nTri_mem, nC_mem)
    ...
    IF (par%master) mesh%region_name = region_name
    CALL sync
    ...
    CALL allocate_shared_memory_int_0D( mesh%nC_mem,           mesh%wnC_mem          )
    CALL allocate_shared_memory_int_0D( mesh%nV_mem,           mesh%wnV_mem          )
    CALL allocate_shared_memory_int_0D( mesh%nTri_mem,         mesh%wnTri_mem        )
    ...
    CALL allocate_shared_memory_dp_0D(  mesh%res_max,          mesh%wres_max         )
    CALL allocate_shared_memory_dp_0D(  mesh%res_max_margin,   mesh%wres_max_margin  )
    CALL allocate_shared_memory_dp_0D(  mesh%res_max_gl,       mesh%wres_max_gl      )
    CALL allocate_shared_memory_dp_0D(  mesh%res_max_cf,       mesh%wres_max_cf      )
    CALL allocate_shared_memory_dp_0D(  mesh%res_max_mountain, mesh%wres_max_mountain)
    CALL allocate_shared_memory_dp_0D(  mesh%res_max_coast,    mesh%wres_max_coast   )
    CALL allocate_shared_memory_dp_0D(  mesh%res_min,          mesh%wres_min         )
    ...
    CALL allocate_shared_memory_dp_2D(  nV_mem,   2,      mesh%V,               mesh%wV              )
    CALL allocate_shared_memory_int_1D( nV_mem,           mesh%nC,              mesh%wnC             )
    CALL allocate_shared_memory_int_2D( nV_mem,   nC_mem, mesh%C,               mesh%wC              )
    CALL allocate_shared_memory_int_1D( nV_mem,           mesh%niTri,           mesh%wniTri          )
    CALL allocate_shared_memory_int_2D( nV_mem,   nC_mem, mesh%iTri,            mesh%wiTri           )
    CALL allocate_shared_memory_int_1D( nV_mem,           mesh%edge_index,      mesh%wedge_index     )
    CALL allocate_shared_memory_int_1D( nV_mem,           mesh%mesh_old_ti_in,  mesh%wmesh_old_ti_in )
    
    CALL allocate_shared_memory_int_2D( nTri_mem, 3,      mesh%Tri,             mesh%wTri            )
    CALL allocate_shared_memory_dp_2D(  nTri_mem, 2,      mesh%Tricc,           mesh%wTricc          )
    CALL allocate_shared_memory_int_2D( nTri_mem, 3,      mesh%TriC,            mesh%wTriC           )
    CALL allocate_shared_memory_int_1D( nTri_mem,         mesh%Tri_edge_index,  mesh%wTri_edge_index )
    
    CALL allocate_shared_memory_int_2D( nTri_mem, 2,      mesh%Triflip,         mesh%wTriflip        )
    CALL allocate_shared_memory_int_1D( nTri_mem,         mesh%RefMap,          mesh%wRefMap         )
    CALL allocate_shared_memory_int_1D( nTri_mem,         mesh%RefStack,        mesh%wRefStack       )
    CALL allocate_shared_memory_int_0D(                   mesh%RefStackN,       mesh%wRefStackN      )
    
    ! Distributed shared memory for FloodFill maps and stacks
    CALL allocate_shared_memory_dist_int_1D( nV_mem,      mesh%VMap,            mesh%wVMap           )
    CALL allocate_shared_memory_dist_int_1D( nV_mem,      mesh%VStack1,         mesh%wVStack1        )
    CALL allocate_shared_memory_dist_int_1D( nV_mem,      mesh%VStack2,         mesh%wVStack2        )
    CALL allocate_shared_memory_dist_int_1D( nTri_mem,    mesh%TriMap,          mesh%wTriMap         )
    CALL allocate_shared_memory_dist_int_1D( nTri_mem,    mesh%TriStack1,       mesh%wTriStack1      )
    CALL allocate_shared_memory_dist_int_1D( nTri_mem,    mesh%TriStack2,       mesh%wTriStack2      )
  ...
  END SUBROUTINE AllocateMesh_primary
  SUBROUTINE ExtendMesh_primary(         mesh, nV_mem_new, nTri_mem_new)
    ...
    CALL adapt_shared_memory_dp_2D(  mesh%nV,   nV_mem_new,    2,           mesh%V,              mesh%wV             )
    CALL adapt_shared_memory_dp_2D(  mesh%nV,   nV_mem_new,    2,           mesh%V,              mesh%wV             )
    CALL adapt_shared_memory_dp_2D(  mesh%nV,   nV_mem_new,    2,           mesh%V,              mesh%wV             )
    CALL adapt_shared_memory_int_1D( mesh%nV,   nV_mem_new,                 mesh%nC,             mesh%wnC            )
    CALL adapt_shared_memory_int_2D( mesh%nV,   nV_mem_new,    mesh%nC_mem, mesh%C,              mesh%wC             )   
    CALL adapt_shared_memory_int_1D( mesh%nV,   nV_mem_new,                 mesh%niTri,          mesh%wniTri         )
    CALL adapt_shared_memory_int_2D( mesh%nV,   nV_mem_new,    mesh%nC_mem, mesh%iTri,           mesh%wiTri          )
    CALL adapt_shared_memory_int_1D( mesh%nV,   nV_mem_new,                 mesh%edge_index,     mesh%wedge_index    )
    CALL adapt_shared_memory_int_1D( mesh%nV,   nV_mem_new,                 mesh%mesh_old_ti_in, mesh%wmesh_old_ti_in)
    
    IF (par%master) mesh%mesh_old_ti_in(mesh%nV+1:nV_mem_new) = 1
    
    CALL adapt_shared_memory_int_2D( mesh%nTri, nTri_mem_new, 3,            mesh%Tri,            mesh%wTri           )
    CALL adapt_shared_memory_dp_2D(  mesh%nTri, nTri_mem_new, 2,            mesh%Tricc,          mesh%wTricc         )    
    CALL adapt_shared_memory_int_2D( mesh%nTri, nTri_mem_new, 3,            mesh%TriC,           mesh%wTriC          )  
    CALL adapt_shared_memory_int_1D( mesh%nTri, nTri_mem_new,               mesh%Tri_edge_index, mesh%wTri_edge_index)
    
    CALL adapt_shared_memory_int_2D( mesh%nTri, nTri_mem_new, 2,            mesh%Triflip,        mesh%wTriflip       )
    CALL adapt_shared_memory_int_1D( mesh%nTri, nTri_mem_new,               mesh%RefMap,         mesh%wRefMap        )
    CALL adapt_shared_memory_int_1D( mesh%nTri, nTri_mem_new,               mesh%RefStack,       mesh%wRefStack      )
    
    ! Distributed shared memory for FloodFill maps and stacks
    CALL adapt_shared_memory_dist_int_1D( mesh%nV,   nV_mem_new,            mesh%VMap,           mesh%wVMap          )
    CALL adapt_shared_memory_dist_int_1D( mesh%nV,   nV_mem_new,            mesh%VStack1,        mesh%wVStack1       )
    CALL adapt_shared_memory_dist_int_1D( mesh%nV,   nV_mem_new,            mesh%VStack2,        mesh%wVStack2       )
    CALL adapt_shared_memory_dist_int_1D( mesh%nTri, nTri_mem_new,          mesh%TriMap,         mesh%wTriMap        )
    CALL adapt_shared_memory_dist_int_1D( mesh%nTri, nTri_mem_new,          mesh%TriStack1,      mesh%wTriStack1     )
    CALL adapt_shared_memory_dist_int_1D( mesh%nTri, nTri_mem_new,          mesh%TriStack2,      mesh%wTriStack2     )
    
  END SUBROUTINE ExtendMesh_primary
  SUBROUTINE CropMesh_primary(           mesh)
    ...
    CALL adapt_shared_memory_int_2D( mesh%nV,   mesh%nV,   mesh%nC_mem,   mesh%C,              mesh%wC             )   
    CALL adapt_shared_memory_int_1D( mesh%nV,   mesh%nV,                   mesh%niTri,          mesh%wniTri         )
    CALL adapt_shared_memory_int_2D( mesh%nV,   mesh%nV,   mesh%nC_mem,   mesh%iTri,           mesh%wiTri          )
  ...
  END SUBROUTINE CropMesh_primary
  SUBROUTINE AllocateMesh_secondary(     mesh)
    ! Allocate memory for mesh data    
    ...
    CALL allocate_shared_memory_dp_2D(  nV,     mesh%nC_mem, mesh%Cw,              mesh%wCw             )
    ...
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nx,              mesh%wNx             )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Ny,              mesh%wNy             )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nxx,             mesh%wNxx            )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nxy,             mesh%wNxy            )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nyy,             mesh%wNyy            )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nxm,             mesh%wNxm            )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nym,             mesh%wNym            )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nxxm,            mesh%wNxxm           )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nxym,            mesh%wNxym           )
    CALL allocate_shared_memory_dp_2D(  nV,   mesh%nC_mem+1, mesh%Nyym,            mesh%wNyym           )
  ...
  END SUBROUTINE AllocateMesh_secondary
  SUBROUTINE DeallocateMesh_all(         mesh)
    ...
    CALL deallocate_shared_memory( mesh%wnC_mem)
    CALL deallocate_shared_memory( mesh%wnV_mem)
    CALL deallocate_shared_memory( mesh%wnTri_mem)
    ...
    CALL deallocate_shared_memory( mesh%wres_max)
    CALL deallocate_shared_memory( mesh%wres_max_margin)
    CALL deallocate_shared_memory( mesh%wres_max_gl)
    CALL deallocate_shared_memory( mesh%wres_max_cf)
    CALL deallocate_shared_memory( mesh%wres_max_mountain)
    CALL deallocate_shared_memory( mesh%wres_max_coast)
    CALL deallocate_shared_memory( mesh%wres_min)
   ...
   END SUBROUTINE DeallocateMesh_all
  
  SUBROUTINE AllocateSubmesh_primary(    mesh, region_name, nV_mem, nTri_mem, nC_mem)
    ! Allocate memory for mesh data    
    ...
    CALL allocate_shared_memory_dist_int_0D( mesh%nC_mem,           mesh%wnC_mem          )
    CALL allocate_shared_memory_dist_int_0D( mesh%nV_mem,           mesh%wnV_mem          )
    CALL allocate_shared_memory_dist_int_0D( mesh%nTri_mem,         mesh%wnTri_mem        )
    ...
    CALL allocate_shared_memory_dist_dp_0D(  mesh%res_max,          mesh%wres_max         )
    CALL allocate_shared_memory_dist_dp_0D(  mesh%res_max_margin,   mesh%wres_max_margin  )
    CALL allocate_shared_memory_dist_dp_0D(  mesh%res_max_gl,       mesh%wres_max_gl      )
    CALL allocate_shared_memory_dist_dp_0D(  mesh%res_max_cf,       mesh%wres_max_cf      )
    CALL allocate_shared_memory_dist_dp_0D(  mesh%res_max_mountain, mesh%wres_max_mountain)
    CALL allocate_shared_memory_dist_dp_0D(  mesh%res_max_coast,    mesh%wres_max_coast   )
    CALL allocate_shared_memory_dist_dp_0D(  mesh%res_min,          mesh%wres_min         )
    ...
    CALL allocate_shared_memory_dist_dp_2D(  nV_mem,   2,      mesh%V,               mesh%wV              )
    CALL allocate_shared_memory_dist_int_1D( nV_mem,           mesh%nC,              mesh%wnC             )
    CALL allocate_shared_memory_dist_int_2D( nV_mem,   nC_mem, mesh%C,               mesh%wC              )
    CALL allocate_shared_memory_dist_int_1D( nV_mem,           mesh%niTri,           mesh%wniTri          )
    CALL allocate_shared_memory_dist_int_2D( nV_mem,   nC_mem, mesh%iTri,            mesh%wiTri           )
    CALL allocate_shared_memory_dist_int_1D( nV_mem,           mesh%edge_index,      mesh%wedge_index     )
    CALL allocate_shared_memory_dist_int_1D( nV_mem,           mesh%mesh_old_ti_in,  mesh%wmesh_old_ti_in )
    
    CALL allocate_shared_memory_dist_int_2D( nTri_mem, 3,      mesh%Tri,             mesh%wTri            )
    CALL allocate_shared_memory_dist_dp_2D(  nTri_mem, 2,      mesh%Tricc,           mesh%wTricc          )
    CALL allocate_shared_memory_dist_int_2D( nTri_mem, 3,      mesh%TriC,            mesh%wTriC           )
    CALL allocate_shared_memory_dist_int_1D( nTri_mem,         mesh%Tri_edge_index,  mesh%wTri_edge_index )
    
    CALL allocate_shared_memory_dist_int_2D( nTri_mem, 2,      mesh%Triflip,         mesh%wTriflip        )
    CALL allocate_shared_memory_dist_int_1D( nTri_mem,         mesh%RefMap,          mesh%wRefMap         )
    CALL allocate_shared_memory_dist_int_1D( nTri_mem,         mesh%RefStack,        mesh%wRefStack       )
  ...
  END SUBROUTINE AllocateSubmesh_primary
  SUBROUTINE ExtendSubmesh_primary(      mesh, nV_mem_new, nTri_mem_new)
    ! For when we didn't allocate enough. Field by field, copy the data to a temporary array,
    ! deallocate the old field, allocate a new (bigger) one, and copy the data back.    
    ...
    CALL adapt_shared_memory_dist_dp_2D(  mesh%nV,   nV_mem_new,   2,             mesh%V,              mesh%wV             )
    CALL adapt_shared_memory_dist_int_1D( mesh%nV,   nV_mem_new,                  mesh%nC,             mesh%wnC            )
    CALL adapt_shared_memory_dist_int_2D( mesh%nV,   nV_mem_new,   mesh%nC_mem,   mesh%C,              mesh%wC             )   
    CALL adapt_shared_memory_dist_int_1D( mesh%nV,   nV_mem_new,                  mesh%niTri,          mesh%wniTri         )
    CALL adapt_shared_memory_dist_int_2D( mesh%nV,   nV_mem_new,   mesh%nC_mem,   mesh%iTri,           mesh%wiTri          )
    CALL adapt_shared_memory_dist_int_1D( mesh%nV,   nV_mem_new,                  mesh%edge_index,     mesh%wedge_index    )
    CALL adapt_shared_memory_dist_int_1D( mesh%nV,   nV_mem_new,                  mesh%mesh_old_ti_in, mesh%wmesh_old_ti_in)
    
    mesh%mesh_old_ti_in(mesh%nV+1:nV_mem_new) = 1
    
    CALL adapt_shared_memory_dist_int_2D( mesh%nTri, nTri_mem_new, 3,             mesh%Tri,            mesh%wTri           )
    CALL adapt_shared_memory_dist_dp_2D(  mesh%nTri, nTri_mem_new, 2,             mesh%Tricc,          mesh%wTricc         )    
    CALL adapt_shared_memory_dist_int_2D( mesh%nTri, nTri_mem_new, 3,             mesh%TriC,           mesh%wTriC          )  
    CALL adapt_shared_memory_dist_int_1D( mesh%nTri, nTri_mem_new,                mesh%Tri_edge_index, mesh%wTri_edge_index)
    
    CALL adapt_shared_memory_dist_int_2D( mesh%nTri, nTri_mem_new, 2,             mesh%Triflip,        mesh%wTriflip       )
    CALL adapt_shared_memory_dist_int_1D( mesh%nTri, nTri_mem_new,                mesh%RefMap,         mesh%wRefMap        )
    CALL adapt_shared_memory_dist_int_1D( mesh%nTri, nTri_mem_new,                mesh%RefStack,       mesh%wRefStack      )
  ...
  END SUBROUTINE ExtendSubmesh_primary
  SUBROUTINE CropSubmesh_primary(        mesh)
    ! For when we allocated too much. Field by field, copy the data to a temporary array,
    ! deallocate the old field, allocate a new (smaller) one, and copy the data back.    
    ...
    CALL adapt_shared_memory_dist_int_2D( mesh%nV,   mesh%nV,   mesh%nC_mem,   mesh%C,              mesh%wC             )   
    CALL adapt_shared_memory_dist_int_1D( mesh%nV,   mesh%nV,                   mesh%niTri,          mesh%wniTri         )
    CALL adapt_shared_memory_dist_int_2D( mesh%nV,   mesh%nV,   mesh%nC_mem,   mesh%iTri,           mesh%wiTri          )
  ...
  END SUBROUTINE CropSubmesh_primary
  SUBROUTINE DeallocateSubmesh_primary(  mesh)
    ! Deallocate memory for mesh data
   ...
   END SUBROUTINE DeallocateSubmesh_primary
  SUBROUTINE ShareSubmeshAccess( p_left, p_right, submesh, submesh_right)
  ...
  END SUBROUTINE ShareSubmeshAccess
