...
CONTAINS
  ...
  SUBROUTINE run_BMB_model( mesh, ice, BMB)
    ...
    IF (C%do_benchmark_experiment) THEN
      ...
      ELSE
        WRITE(0,*) '  ERROR: benchmark experiment "', TRIM(C%choice_benchmark_experiment), '" not implemented in run_BMB_model!'
...
!    weight = MAX(0._dp, MIN(2._dp, 1._dp + gT_offset/12._dp + MAX(0._dp, (insolation_data%Jun65N - 462.29_dp)/40._dp)))
    ...
    DO vi = mesh%v1, mesh%v2
      ...
      S_flux   = seawater_density * cp0 * sec_per_year * gamma_T * F_melt * & 
                      (T_ocean - T_freeze) / (L_fusion * ice_density)
      z_deep = MAX(0._dp, MIN(1._dp, (ice%SL( vi) - ice%Hb(vi) - 1200._dp)/200._dp))
  ...
  SUBROUTINE initialise_BMB_model( mesh, BMB)
    ...
    CALL allocate_shared_dp_1D( mesh%nV,     BMB%ocean_intrusion,  BMB%wocean_intrusion )
