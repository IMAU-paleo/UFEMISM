...
!    IF (C%choice_ice_dynamics == 'none') THEN
!      ! Fixed ice geometry
!      region%ice%dHi_dt_a( v1:v2) = 0._dp
!      CALL sync
!    ELSE
!      CALL calc_dHi_dt( region%mesh, region%ice, region%SMB, region%BMB, region%dt, region%mask_noice, region%refgeo_PD, region%refgeo_GIAeq)
!    END IF
!    
...
!      
!      ! Calculate new ice geometry
!      region%ice%pc_f2(   v1:v2) = region%ice%dHi_dt_a( v1:v2)
!      CALL calc_dHi_dt( region%mesh, region%ice, region%SMB, region%BMB, region%dt_crit_ice, region%mask_noice, region%refgeo_PD, region%refgeo_GIAeq)
!      region%ice%pc_f1(   v1:v2) = region%ice%dHi_dt_a( v1:v2)
!      region%ice%Hi_pred( v1:v2) = MAX(0._dp, region%ice%Hi_a(     v1:v2) + region%dt_crit_ice * region%ice%dHi_dt_a( v1:v2))
!      CALL sync
!      
!      ! Update step
!      ! ===========
...
!      region%ice%Hi_old( v1:v2) = region%ice%Hi_a(    v1:v2)
!      region%ice%Hi_a(   v1:v2) = region%ice%Hi_pred( v1:v2)
...
!      region%ice%Hi_a(    v1:v2) = region%ice%Hi_old(   v1:v2)
!      CALL update_general_ice_model_data( region%mesh, region%ice)
!      
!      ! Calculate "corrected" ice thickness based on new velocities
!      CALL calc_dHi_dt( region%mesh, region%ice, region%SMB, region%BMB, region%dt_crit_ice, region%mask_noice, region%refgeo_PD, region%refgeo_GIAeq)
!      region%ice%pc_f3(   v1:v2) = region%ice%dHi_dt_a( v1:v2)
!      region%ice%pc_f4(   v1:v2) = region%ice%pc_f1(    v1:v2)
!      region%ice%Hi_corr( v1:v2) = MAX(0._dp, region%ice%Hi_a( v1:v2) + 0.5_dp * region%dt_crit_ice * (region%ice%pc_f3( v1:v2) + region%ice%pc_f4( v1:v2)))
!      CALL sync
!  
...
!    region%ice%Hi_tplusdt_a( v1:v2) = MAX( 0._dp, region%ice%Hi_a( v1:v2) + region%dt * region%ice%dHi_dt_a( v1:v2))
!    CALL sync
!    
...
! == Update the ice thickness at the end of a model time loop
  SUBROUTINE update_ice_thickness( mesh, ice)
    ...
    ice%mask_ice_a_prev( mesh%v1:mesh%v2) = ice%mask_ice_a( mesh%v1:mesh%v2)
    CALL sync
    
    ! Set ice thickness to new value
    ice%Hi_a( mesh%v1:mesh%v2) = MAX( 0._dp, ice%Hi_tplusdt_a( mesh%v1:mesh%v2))
    CALL sync
...
! == Time stepping
  SUBROUTINE calc_critical_timestep_SIA( mesh, ice, dt_crit_SIA)
    ...
    DO aci = mesh%ac1, mesh%ac2
      ...
      u_SIA_vav = vertical_average( ice%u_3D_SIA_c( :,aci))
      v_SIA_vav = vertical_average( ice%v_3D_SIA_c( :,aci))
      D_SIA = 1E-9_dp
      ...
      DO k = 1, C%nz
        ...
        dt = dist / (ABS( ice%u_3D_c( k,aci)) + ABS( ice%v_3D_c( k,aci)))
        dt_crit_SIA = MIN( dt_crit_SIA, dt)
...
! == Administration: allocation, initialisation, and remapping
  SUBROUTINE initialise_ice_model( mesh, ice, init)
    ...
    ice%Hi_a( mesh%v1:mesh%v2) = init%Hi( mesh%v1:mesh%v2)
    ice%Hb_a( mesh%v1:mesh%v2) = init%Hb( mesh%v1:mesh%v2)
    ice%Hs_a( mesh%v1:mesh%v2) = MAX( ice%SL_a( mesh%v1:mesh%v2), ice%Hb_a( mesh%v1:mesh%v2) + ice%Hi_a( mesh%v1:mesh%v2))
    CALL sync
    ...
    IF     (C%choice_geothermal_heat_flux == 'constant') THEN
      ice%GHF_a( mesh%v1:mesh%v2) = C%constant_geothermal_heat_flux
  ...
  SUBROUTINE remap_ice_model( mesh_old, mesh_new, map, ice, PD)
    ...
    ice%Hb_a( mesh_new%v1:mesh_new%v2) = PD%Hb( mesh_new%v1:mesh_new%v2) + ice%dHb_a( mesh_new%v1:mesh_new%v2)
...
!    ice%V_SSA( mesh_new%v1:mesh_new%v2) = ice%V_SSA( mesh_new%v1:mesh_new%v2) * 0.9_dp
    ...
    IF     (C%choice_geothermal_heat_flux == 'constant') THEN
      ice%GHF_a( mesh_new%v1:mesh_new%v2) = C%constant_geothermal_heat_flux
    ELSEIF (C%choice_geothermal_heat_flux == 'spatial') THEN
