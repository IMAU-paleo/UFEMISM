MODULE data_types_module
  ...
  TYPE type_remapping_mesh2grid
    ...
    INTEGER,  DIMENSION(:    ), POINTER :: mesh_nV    ! Number    of grid cells contributing to a vertex (if smaller then mesh%nC_mem)
    INTEGER,  DIMENSION(:,:  ), POINTER :: mesh_i     ! i-indices of grid cells contributing to a vertex (if smaller then mesh%nC_mem)
    INTEGER,  DIMENSION(:,:  ), POINTER :: mesh_j     ! i-indices of grid cells contributing to a vertex (if smaller then mesh%nC_mem)
    REAL(dp), DIMENSION(:,:  ), POINTER :: mesh_w_g2m ! Weights for mapping from the grid to the mesh
    REAL(dp), DIMENSION(:,:  ), POINTER :: mesh_w_m2g ! Weights for mapping from the mesh to the grid
    INTEGER,  DIMENSION(:,:  ), POINTER :: grid_nV    ! Number  of vertices contributing to a grid cell (if the vertex contains more than mesh%nC_mem grid cells)
    INTEGER,  DIMENSION(:,:,:), POINTER :: grid_vi    ! Indices of vertices contributing to a grid cell (if the vertex contains more than mesh%nC_mem grid cells)
    REAL(dp), DIMENSION(:,:,:), POINTER :: grid_w_g2m ! Weights for mapping from the grid to the mesh
    REAL(dp), DIMENSION(:,:,:), POINTER :: grid_w_m2g ! Weights for mapping from the mesh to the grid
    REAL(dp), DIMENSION(:,:  ), POINTER :: grid_w_tot ! Normalisation weights for mapping from the mesh to the grid
    INTEGER :: wmesh_nV, wmesh_i, wmesh_j, wmesh_w_g2m, wmesh_w_m2g, wgrid_nV, wgrid_vi, wgrid_w_g2m, wgrid_w_m2g, wgrid_w_tot
