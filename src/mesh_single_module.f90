...
contains
  ...
  SUBROUTINE create_single_mesh_from_cart_data( region)
    ...
    DO WHILE (res_min_inc > C%res_min)
      ...
      if (debug_mesh_creation) then
        write(*,"(A,I3,A,F6.1,A)") '   Process ', par%i, ' refining submesh to ', submesh%res_max_gl, ' km...'
      end if

      ! Refine the process submesh
      CALL refine_mesh( submesh, region%refgeo_init)

      ! Split any new triangles that are too sharp
    ...
    IF (debug_mesh_creation .AND. par%master) WRITE(0,*) '   Creating new mesh...'
  ...
  SUBROUTINE create_new_mesh_single( region)
    ...
    IF (debug_mesh_creation .AND. par%master) WRITE(0,*) '   Creating a new mesh...'
    ...
    IF (par%master) THEN
      ...
      write(*,"(A)")               '   Finished creating the new mesh.'
    END IF
