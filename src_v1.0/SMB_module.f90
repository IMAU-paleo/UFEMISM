...
CONTAINS
  ...
  SUBROUTINE run_SMB_model(mesh, ice, climate, region_time, SMB)
    ...
    ! If we're doing one of the EISMINT experiments, use the SMB parameterisation
    IF (C%do_eismint_experiment) THEN
      CALL EISMINT_SMB(mesh, ice, region_time, SMB)
      RETURN
    ELSEIF (C%do_mismip3d_experiment) THEN
      SMB%SMB_year = 0.5_dp
      SMB%SMB      = 0.5_dp / 12._dp
      RETURN
    END IF
    ...
    wt0 = (forcing%ins_t1 * 1000._dp - region_time) / ((forcing%ins_t1 - forcing%ins_t0) * 1000._dp)
    wt1 = 1._dp - wt0
  ...
  SUBROUTINE EISMINT_SMB(mesh, ice, time, SMB)
    ! Run the IMAU-ITM SMB model. Based on the one from ANICE.
    ...
    ! Local variables:
    INTEGER                                            :: vi
    REAL(dp)                                           :: kilo = 1000._dp ! Factor used for kilometer to meter conversion
    ...
    REAL(dp)                                           :: X_center        ! X-coordinate of center of MB (in m)
    REAL(dp)                                           :: Y_center        ! Y-coordinate of center of MB (in m)
    ...
    SELECT CASE(C%choice_eismint_experiment)
    CASE ('A', 'B', 'F', 'G', 'H', 'M')
      ! no changes, except for temperature for B
    CASE ('C')                                          ! Experiment C
      E         = 425._dp * kilo 
      M_max     = 0.25_dp
    CASE ('D')                                          ! Experiment D
      E         = 425._dp * kilo 
    CASE ('E')                                          ! Experiment E
      X_center  = X_center + 100._dp * kilo
      Y_center  = Y_center + 100._dp * kilo
    CASE ('N')                                          ! Experiment EISMINT 1N moving margin; sinusoidal M 20kyr
      IF (time < 0._dp) THEN
        E         = 450._dp * kilo
      ELSE
        E         = 450._dp * kilo + 100._dp * kilo * SIN(2._dp*C%pi*time/20000._dp)
      END IF
    CASE ('O')                                          ! Experiment EISMINT 1O moving margin; sinusoidal M 40kyr
      IF (time < 0._dp) THEN
        E         = 450._dp * kilo
      ELSE
        E         = 450._dp * kilo + 100._dp * kilo * SIN(2._dp*C%pi*time/40000._dp)
      END IF
    CASE ('P')                                          ! Experiment EISMINT 1P fixed margin; constant M by setting E outside domain
      M_max     = 0.3_dp       
      E         = 999._dp * kilo 
    CASE ('Q')                                          ! Experiment EISMINT 1Q fixed margin; sinusoidal M 20kyr
      IF (time < 0._dp) THEN
        M_max     = 0.3_dp
        E         = 999._dp * kilo 
      ELSE
        M_max     = 0.3_dp + 0.2_dp * SIN(2._dp*C%pi*time/20000._dp)
        E         = 999._dp * kilo 
      END IF
    CASE ('R')                                          ! Experiment EISMINT 1R fixed margin; sinusoidal M 40kyr
      IF (time < 0._dp) THEN
        M_max     = 0.3_dp
        E         = 999._dp * kilo 
      ELSE
        M_max     = 0.3_dp + 0.2_dp * SIN(2._dp*C%pi*time/40000._dp)
        E         = 999._dp * kilo 
      END IF
    CASE DEFAULT
      WRITE(6,*) "C%choice_eismint_experiment should be A-H or M-R"
      STOP
    END SELECT

    DO vi = mesh%v1, mesh%v2
      dist = SQRT( (mesh%V(vi,1) - X_center)**2 + (mesh%V(vi,2) - Y_center)**2)
  ...
  SUBROUTINE initialise_SMB_model(mesh, climate, init, filetype_init, SMB)
    ...
    ! Allocate memory
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%Q_TOA,            SMB%wQ_TOA           )
    CALL allocate_shared_memory_dp_1D( mesh%nV,     SMB%AlbedoSurf,       SMB%wAlbedoSurf      )
    CALL allocate_shared_memory_dp_1D( mesh%nV,     SMB%MeltPreviousYear, SMB%wMeltPreviousYear)
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%FirnDepth,        SMB%wFirnDepth       )
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%Rainfall,         SMB%wRainfall        )
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%Snowfall,         SMB%wSnowfall        )
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%AddedFirn,        SMB%wAddedFirn       )
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%Melt,             SMB%wMelt            )
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%Refreezing,       SMB%wRefreezing      )
    CALL allocate_shared_memory_dp_1D( mesh%nV,     SMB%Refreezing_year,  SMB%wRefreezing_year )
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%Runoff,           SMB%wRunoff          )
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%Albedo,           SMB%wAlbedo          )
    CALL allocate_shared_memory_dp_2D( mesh%nV, 12, SMB%SMB,              SMB%wSMB             )
    CALL allocate_shared_memory_dp_1D( mesh%nV,     SMB%SMB_year,         SMB%wSMB_year        )
