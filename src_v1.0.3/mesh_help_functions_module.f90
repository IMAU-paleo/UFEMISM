...
!    IF (SIZE(mesh%V)              /= mesh%nV   * 2           ) WRITE(0,*) ' check_mesh - ERROR: array V              has incorrect size!'
!    IF (SIZE(mesh%nC)             /= mesh%nV   * 1           ) WRITE(0,*) ' check_mesh - ERROR: array nC             has incorrect size!'
!    IF (SIZE(mesh%C)              /= mesh%nV   * mesh%nC_mem ) WRITE(0,*) ' check_mesh - ERROR: array C              has incorrect size!'
!    IF (SIZE(mesh%niTri)          /= mesh%nV   * 1           ) WRITE(0,*) ' check_mesh - ERROR: array niTri          has incorrect size!'
!    IF (SIZE(mesh%iTri)           /= mesh%nV   * mesh%nC_mem ) WRITE(0,*) ' check_mesh - ERROR: array iTri           has incorrect size!'
!    IF (SIZE(mesh%edge_index)     /= mesh%nV   * 1           ) WRITE(0,*) ' check_mesh - ERROR: array edge_index     has incorrect size!'
!    
!    IF (SIZE(mesh%Tri)            /= mesh%nTri * 3           ) WRITE(0,*) ' check_mesh - ERROR: array Tri            has incorrect size!'
!    IF (SIZE(mesh%Tricc)          /= mesh%nTri * 2           ) WRITE(0,*) ' check_mesh - ERROR: array Tricc          has incorrect size!'
!    IF (SIZE(mesh%TriC)           /= mesh%nTri * 3           ) WRITE(0,*) ' check_mesh - ERROR: array TriC           has incorrect size!'
!    IF (SIZE(mesh%Tri_edge_index) /= mesh%nTri * 1           ) WRITE(0,*) ' check_mesh - ERROR: array Tri_edge_index has incorrect size!'
        
    ! == V
    ! =============================================================
    DO vi = 1, mesh%nV
      IF (mesh%V(vi,1) < mesh%xmin .OR. mesh%V(vi,1) > mesh%xmax .OR. mesh%V(vi,2) < mesh%ymin .OR. mesh%V(vi,2) > mesh%ymax) THEN
        WRITE(0,*) ' check_mesh - ERROR: vertex ', vi, ' outside mesh domain!'
      END IF
    END DO
