MODULE mesh_update_module
  ...
  USE mesh_help_functions_module,  ONLY: Cart_Bilinear_dp, MaxCartOverTriangle_dp, Mesh_Bilinear_dp, IsInTriangle, &
                                         PartitionDomain_regular, PartitionDomain_X_balanced, PartitionDomain_Y_balanced, &
                                         NewTriangleContainsOldMask, WriteMeshToTextFile, IsBoundarySegment, IsEncroachedUpon
  USE mesh_memory_module,          ONLY: AllocateSubmesh_primary, ExtendSubmesh_primary
  USE mesh_Delaunay_module,        ONLY: SplitTriangle
  USE mesh_derivatives_module,     ONLY: GetMeshCurvaturesVertex
  USE mesh_creation_module,        ONLY: InitialiseDummyMesh, PerturbDummyMesh, AlignAllSubmeshes, RefineSubmesh_geo_only, &
                                         MergeAllSubmeshes, CreateFinalMeshFromMergedSubmesh
  ...
  SUBROUTINE IsGoodTriangle( mesh_new, ti, mesh_old, ice, PD, IsGood)
    ...
    DO n = 1, mesh_new%nPOI
      ...
      IF (IsInTriangle( p, q, r, POI) .AND. dmax > mesh_new%POI_resolutions(n) * 1.5_dp * 1000._dp) THEN
        IsGood = .FALSE.
        RETURN
      END IF
    END DO
  ...
  SUBROUTINE CreateNewMesh( region)
    ...
    IF (orientation == 0) THEN
      CALL PartitionDomain_regular( region%mesh%xmin, region%mesh%xmax, par%i, par%n, xmin, xmax)
!      CALL PartitionDomain_X_balanced( region%mesh, region%mesh%ymin, region%mesh%ymax, par%i, par%n, xmin, xmax)
      ymin = region%mesh%ymin
      ymax = region%mesh%ymax
    ELSE
      CALL PartitionDomain_regular( region%mesh%ymin, region%mesh%ymax, par%i, par%n, ymin, ymax)
!      CALL PartitionDomain_Y_balanced( region%mesh, region%mesh%xmin, region%mesh%xmax, par%i, par%n, ymin, ymax)
    ...
    DO WHILE (res_min_inc > C%res_min)
      ...
      CALL RefineMesh( submesh, region%mesh, region%ice, region%PD) 
  ...
  SUBROUTINE RefineMesh( mesh, mesh_old, ice, PD)
    ...
    DO WHILE (.NOT. FinishedRefining)
      ...
      DO WHILE (mesh%RefStackN > 0)
        ...
        CALL IsGoodTriangle(mesh, ti, mesh_old, ice, PD, IsGood)
