MODULE mesh_creation_module
  ...
  USE mesh_help_functions_module,    ONLY: find_connection_widths, find_triangle_areas, find_Voronoi_cell_areas, get_lat_lon_coordinates, &
                                          determine_mesh_resolution, write_mesh_to_screen, merge_vertices, switch_vertices, redo_Tri_edge_indices, check_mesh, &
                                           cart_bilinear_dp, cart_bilinear_int, max_cart_over_triangle_int, max_cart_over_triangle_dp, &
                                           min_cart_over_triangle_int, sum_cart_over_triangle_dp, is_in_triangle, segment_intersection, &
  ...
  USE mesh_Delaunay_module,          ONLY: split_triangle, split_segment, flip_triangle_pairs
  ...
  SUBROUTINE is_good_triangle( mesh, ti, init, IsGood)
    ...
    CALL min_cart_over_triangle_int( p, q, r, init%mask_ice_cart, init%x, init%y, init%nx, init%ny, min_mask_int, trinel)
    CALL max_cart_over_triangle_int( p, q, r, init%mask_ice_cart, init%x, init%y, init%nx, init%ny, max_mask_int, trinel)    
    IF (trinel>0) THEN
    ...
    CALL max_cart_over_triangle_int(p,q,r, init%mask_gl_cart, init%x, init%y, init%nx, init%ny, max_mask_int, trinel)    
    IF (trinel>0) THEN
    ...
    CALL max_cart_over_triangle_int(p,q,r, init%mask_cf_cart, init%x, init%y, init%nx, init%ny, max_mask_int, trinel)    
    IF (trinel>0) THEN
    ...
    CALL max_cart_over_triangle_int(p,q,r, init%mask_coast_cart, init%x, init%y, init%nx, init%ny, max_mask_int, trinel)    
    IF (trinel>0) THEN
    ...
    CALL sum_cart_over_triangle_dp(p,q,r, init%surface_curvature_cart, init%x, init%y, init%nx, init%ny, trisumel, trinel)
    IF (trinel>4) THEN
    ...
    CALL max_cart_over_triangle_dp(p,q,r, init%Hb_cart, init%x, init%y, init%nx, init%ny, Hb_max,trinel)
    IF (trinel==0) THEN
  ...
  SUBROUTINE create_mesh_from_cart_data(region)
    ...
    CALL partition_domain_regular( MINVAL(region%init%x), MAXVAL(region%init%x), par%i, par%n, xmin, xmax)
...
!      WRITE(0,'(A,I2,A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': merging mesh ', p_left, ' with mesh ', p_right
    ELSE
!      WRITE(0,'(A,I2,A)')         ' merge_submeshes - process ', par%i, ': passively running merge_submeshes'
    END IF
    CALL sync
  ...
  ! =================================================  
    ...
    IF (par%i == p_left .OR. par%i == p_right) THEN
!      IF (par%i == p_left)  WRITE(0,'(A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': gaining   access to submesh memory from process ', p_right
!      IF (par%i == p_right) WRITE(0,'(A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': providing access to submesh memory to   process ', p_left
...
!      WRITE(0,'(A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': recalculating T'
...
!      WRITE(0,'(A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': adding data from p_right to p_left'     
...
!      WRITE(0,'(A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': merging shared vertices'  
...
!      WRITE(0,'(A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': redoing triangle edge indices'  
...
!      WRITE(0,'(A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': resetting corner vertices'  
...
!      WRITE(0,'(A,I2,A,I2)') ' merge_submeshes - process ', par%i, ': updating Delaunay triangulation'  
      ...
      DO ti = 1, nT
        ...
        ! Flip triangle pairs
        DO WHILE (nf > 0)
          CALL flip_triangle_pairs( submesh, nf)
        END DO
