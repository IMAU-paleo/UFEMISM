MODULE ice_dynamics_module
  ...
  USE mesh_ArakawaC_module,          ONLY: MapAaToAc, MapAcToAa
...
!        END IF
  ...
  SUBROUTINE solve_SSA(               ice, mesh)
    ...
    ! In some benchmark experiments, only the SIA is solved.
    ...
    ! Check which sliding law needs to be applied
    ...
    DO outer_loop_i = 1, outer_loop_n
      ...
      DO vi = mesh%v1, mesh%v2
        ...
        IF (ice%mask_sheet(vi)==1) THEN
          sliding_term_x = (C%C_sliding / (C%sec_per_year ** C%m_sliding)) * ((ABS(ice%U_SSA( vi)) + delta_v) ** (C%m_sliding - 1._dp)) / (ice%nu( vi) * ice%Hi( vi))
          sliding_term_y = (C%C_sliding / (C%sec_per_year ** C%m_sliding)) * ((ABS(ice%V_SSA( vi)) + delta_v) ** (C%m_sliding - 1._dp)) / (ice%nu( vi) * ice%Hi( vi))
        ELSE
  ...
  SUBROUTINE allocate_ice_model( ice, mesh)
    ...
    CALL allocate_shared_memory_dp_1D(  mesh%nV,         ice%D_SIA,                 ice%wD_SIA                )
    ...
    CALL allocate_shared_memory_dp_2D(  mesh%nV, mesh%nC_mem,   ice%dVi_in,         ice%wdVi_in               )
    CALL allocate_shared_memory_dp_2D(  mesh%nV, mesh%nC_mem,   ice%dVi_out,        ice%wdVi_out              ) 
  ...
  SUBROUTINE remap_ice_model( mesh_old, mesh_new, map, ice)
    ...
    CALL Remap_field_dp(         mesh_old, mesh_new, map, ice%Hi,                  ice%wHi,                  'cons_2nd_order')
    CALL Remap_field_dp(         mesh_old, mesh_new, map, ice%Hs,                  ice%wHs,                  'trilin'        )
    CALL Remap_field_dp_3D(      mesh_old, mesh_new, map, ice%Ti,                  ice%wTi,                  'cons_2nd_order')
    CALL Remap_field_dp(         mesh_old, mesh_new, map, ice%U_SIA,               ice%wU_SIA,               'trilin'        )
    CALL Remap_field_dp(         mesh_old, mesh_new, map, ice%V_SIA,               ice%wV_SIA,               'trilin'        ) 
    ...
    CALL Remap_field_dp(         mesh_old, mesh_new, map, ice%D_SIA,               ice%wD_SIA,               'trilin'        )
    ...
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%sealevel,               ice%wsealevel                 )
    ...
    CALL Reallocate_field_int(   mesh_new%nV,  ice%mask_groundingline,     ice%wmask_groundingline       ) 
    CALL Reallocate_field_int(   mesh_new%nV,  ice%mask_calvingfront,      ice%wmask_calvingfront        )
    ...
    CALL Reallocate_field_dp(    mesh_new%nAC, ice%sealevel_ac,            ice%wsealevel_ac              )
    ...
    CALL Reallocate_field_int(   mesh_new%nAC, ice%mask_groundingline_ac,  ice%wmask_groundingline_ac    ) 
    CALL Reallocate_field_int(   mesh_new%nAC, ice%mask_calvingfront_ac,   ice%wmask_calvingfront_ac     ) 
    CALL Reallocate_field_dp_3D( mesh_new%nAC, ice%Ti_ac,                  ice%wTi_ac              , C%nz)
    CALL Reallocate_field_dp_3D( mesh_new%nAC, ice%Ti_pmp_ac,              ice%wTi_pmp_ac          , C%nz)
    ...
    CALL Reallocate_field_dp_3D( mesh_new%nAC, ice%Cpi_ac,                 ice%wCpi_ac             , C%nz)
    CALL Reallocate_field_dp_3D( mesh_new%nAC, ice%Ki_ac,                  ice%wKi_ac              , C%nz)
    ...
    CALL Reallocate_field_dp(    mesh_new%nAC, ice%Ux_ac,                  ice%wUx_ac                    )
    CALL Reallocate_field_dp(    mesh_new%nAC, ice%Uy_ac,                  ice%wUy_ac                    )
    CALL Reallocate_field_dp(    mesh_new%nAC, ice%Up_ac,                  ice%wUp_ac                    )
    CALL Reallocate_field_dp(    mesh_new%nAC, ice%Uo_ac,                  ice%wUo_ac                    )
    
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%tau_yield,              ice%wtau_yield                )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%dUdx,                   ice%wdUdx                     )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%dUdy,                   ice%wdUdy                     )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%dVdx,                   ice%wdVdx                     )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%dVdy,                   ice%wdVdy                     )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%nu,                     ice%wnu                       )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%beta_base,              ice%wbeta_base                )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%rhs_x,                  ice%wrhs_x                    )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%rhs_y,                  ice%wrhs_y                    )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%eu_i,                   ice%weu_i                     )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%ev_i,                   ice%wev_i                     )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%Rx,                     ice%wRx                       )
    CALL Reallocate_field_dp(    mesh_new%nV,  ice%Ry,                     ice%wRy                       )    
