...
! == Routines for partitioning a domain or list, either regularly or load-balanced
  SUBROUTINE PartitionDomain_regular( xmin, xmax, i, n, xl, xr)
  ...
  END SUBROUTINE PartitionDomain_regular
  SUBROUTINE PartitionDomain_X_balanced( mesh, ymin, ymax, i, n, xmin, xmax)
  ...
  END SUBROUTINE PartitionDomain_X_balanced
  SUBROUTINE PartitionDomain_Y_balanced( mesh, xmin, xmax, i, n, ymin, ymax)
  ...
  END SUBROUTINE PartitionDomain_Y_balanced
  SUBROUTINE PartitionList( ntot, i, n, i1, i2)
...
! == Subroutines for creating Points of Interest (POI)
  SUBROUTINE FindPOIXYCoordinates( mesh)
    ...
    ELSE
      ...
      WRITE(0,*) '  ERROR - region name unknown!'
  ...
  ! Called by all, executed by the master.    
    ...
    CHARACTER(LEN=256), PARAMETER                 :: caller_name = 'FindPOIVerticesAndWeights'
    ...
    IF (par%master) THEN
      ...
      DO POIi = 1, mesh%nPOI
        ...
        CALL FindContainingTriangle( mesh, p, ti, caller_name)
...
! == Some basic search operations on a mesh
  SUBROUTINE FindContainingTriangle( mesh, p, t, caller_name)
   ...
   ! flood-fill algorithm.    
    ...
    CHARACTER(LEN=256),       INTENT(IN)          :: caller_name
    ...
    IF (p(1) < mesh%xmin .OR. p(1) > mesh%xmax .OR. p(2) < mesh%ymin .OR. p(2) > mesh%ymax) THEN
      WRITE(0,*) 'FindContainingTriangle (called by ', TRIM(caller_name), ') - ERROR: point lies outside mesh domain!'
      STOP
    END IF
    ...
    DO WHILE (.NOT. FoundIt)
      ...
      IF (mesh%TriStackN2==0) THEN
        WRITE(0,*) 'FindContainingTriangle (called by ', caller_name, ') - ERROR: couldnt find triangle containing this point!'
        STOP
      END IF
...
! == Help routines for use in mesh updating
  ...
  SUBROUTINE Mesh_Bilinear_dp(  mesh, d, p, ti, v)
    ...
    CHARACTER(LEN=256), PARAMETER                 :: caller_name = 'Mesh_Bilinear_dp'
  ...
  SUBROUTINE Mesh_Bilinear_int( mesh, d, p, ti, v)
    ...
    INTEGER                                       :: v1, v2, v3
    REAL(dp)                                      :: dx, dy, ddx, ddy
    CHARACTER(LEN=256), PARAMETER                 :: caller_name = 'Mesh_Bilinear_int'
  ...
  SUBROUTINE NewTriangleContainsOldMask( mesh, pa, pb, pc, mask, vi_closest_to_gc, isso)
    ...
    CHARACTER(LEN=256), PARAMETER                 :: caller_name = 'NewTriangleContainsOldMask'
    ...
    IF (.NOT. IsInTriangle( pa, pb, pc, p)) THEN
      ...
      CALL FindContainingTriangle( mesh, gc, ti_containing_gc, caller_name)
...
!    IF (SIZE(mesh%C)              /= mesh%nV   * mesh%nC_mem ) WRITE(0,*) ' CheckMesh - ERROR: array C              has incorrect size!'
!    IF (SIZE(mesh%niTri)          /= mesh%nV   * 1            ) WRITE(0,*) ' CheckMesh - ERROR: array niTri          has incorrect size!'
!    IF (SIZE(mesh%iTri)           /= mesh%nV   * mesh%nC_mem ) WRITE(0,*) ' CheckMesh - ERROR: array iTri           has incorrect size!'
