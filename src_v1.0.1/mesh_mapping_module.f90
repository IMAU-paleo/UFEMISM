MODULE mesh_mapping_module
  ...
  USE mesh_help_functions_module,  ONLY: IsInTriangle, FindVoronoiCellVertices, FindContainingTriangle, FindTriangleArea, FindContainingVertex, &
                                         LineIntegral_xdy, LineIntegral_mxydx, LineIntegral_xydy, IsboundarySegment, Cross2, &
                                         LiesOnLineSegment, SegmentIntersection, PartitionDomain_X_balanced, WriteMeshToTextFile, &
                                         LineFromPoints, LineLineIntersection
  ...
  SUBROUTINE CreateRemappingArrays_trilin( mesh_src, mesh_dst, map)
    ...
    CHARACTER(LEN=256), PARAMETER                          :: caller_name = 'CreateRemappingArrays_trilin'
    ...
    DO vi_dst = mesh_dst%v1, mesh_dst%v2
      ...
      CALL FindContainingTriangle( mesh_src, p, ti_src, caller_name)
      ...
      Atot = FindTriangleArea( pa, pb, pc)
      Aa   = FindTriangleArea( pb, pc, p )
      Ab   = FindTriangleArea( pc, pa, p )
      Ac   = FindTriangleArea( pa, pb, p )
  ...
  SUBROUTINE CreateRemappingArrays_conservative( mesh_src, mesh_dst, nV_max, map)
    ...
    CALL DetermineProcessDomains( mesh_src, proc_domain_Ac_src, vi_src_start_proc)
    CALL DetermineProcessDomains( mesh_dst, proc_domain_Ac_dst, vi_dst_start_proc)

    ! Integrate over all Voronoi cell boundary lines for both meshes
  
    CountCoincidences = .FALSE.  
    CALL IntegrateOverVoronoiBoundaries( mesh_src, mesh_dst, proc_domain_Ac_src, &
    ...
    CALL IntegrateOverVoronoiBoundaries( mesh_dst, mesh_src, proc_domain_Ac_dst, &
  ...
  SUBROUTINE DetermineProcessDomains( mesh, proc_domain_Ac, vi_start_proc)
    ...
    DO i = 0, par%n-1
      ...
      CALL PartitionDomain_X_balanced( mesh, mesh%ymin, mesh%ymax, i, par%n, xmin, xmax)
  ...
  SUBROUTINE IntegrateOverVoronoiBoundaries( mesh_top, mesh_bot,  proc_domain_Ac_top, &
  ...
  SUBROUTINE CheckIfRemappingIsConservative( mesh_src, mesh_dst, map)
    ...
    REAL(dp), DIMENSION(2)                                 :: p, pa, pb, pc
    REAL(dp)                                               :: Atot, Aa, Ab, Ac
    CHARACTER(LEN=256), PARAMETER                          :: caller_name = 'CheckIfRemappingIsConservative'
...
! == Low-level remapping routines
  ...
  SUBROUTINE Remap_trilin_3D(                           mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using trilinear interpolation
  ...
  SUBROUTINE Remap_nearest_neighbour_3D(                mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using nearest neighbour interpolation
  ...
  SUBROUTINE Remap_cons_1st_order_3D(                   mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using 1st order conservative remapping
  ...
  SUBROUTINE Remap_cons_2nd_order_2D_monthly( mesh_src, mesh_dst, map, d_src, d_dst)
    ...
    CALL GetMeshDerivatives_3D( mesh_src, d_src, ddx_src, ddy_src)
  ...
  SUBROUTINE Remap_cons_2nd_order_3D(         mesh_src, mesh_dst, map, d_src, d_dst)
    ! Remap data from mesh_src to mesh_dst using 2nd order conservative remapping
...
! == Medium-level remapping routines, where the memory containing the data is reallocated
  ...
  SUBROUTINE Remap_field_dp_3D(      mesh_old, mesh_new, map, d, w, method)
    ...
    ! Map data field from old mesh to new mesh
    IF (method == 'trilin') THEN
      CALL Remap_trilin_3D(                      mesh_new, map%trilin,       d_temp, d)      
    ELSEIF (method == 'cons_2nd_order') THEN
      CALL Remap_cons_2nd_order_3D(    mesh_old, mesh_new, map%conservative, d_temp, d)
    ELSE
