MODULE mesh_ArakawaC_module
  ...
  USE mesh_help_functions_module,  ONLY: IsBoundarySegment, PartitionList
  ...
  SUBROUTINE MakeAcMesh(mesh)
    ...
    CALL allocate_shared_memory_int_2D( mesh%nV,     mesh%nC_mem, mesh%iAci,            mesh%wiAci          )
    ...
    CALL PartitionList( mesh%nAc, par%i, par%n, mesh%c1, mesh%c2)
    
  END SUBROUTINE MakeAcMesh
  
  SUBROUTINE GetAcMeshDerivatives( mesh, d, ddx_ac, ddy_ac, ddp_ac, ddo_ac)    
    ...
    DO aci = mesh%c1, mesh%c2
      DO n = 1, 4
        ddx_ac( aci)  = ddx_ac( aci)  + mesh%Nx_ac( aci,n) * d( mesh%Aci( aci,n))
        ddy_ac( aci)  = ddy_ac( aci)  + mesh%Ny_ac( aci,n) * d( mesh%Aci( aci,n))
        ddo_ac( aci)  = ddo_ac( aci)  + mesh%No_ac( aci,n) * d( mesh%Aci( aci,n))
      END DO
      ddp_ac( aci)  = mesh%Np_ac( aci) * (d( mesh%Aci( aci,2)) - d( mesh%Aci( aci,1)))
    END DO ! DO ci = mesh%c1, mesh%c2
    CALL sync
    
  END SUBROUTINE GetAcMeshDerivatives
  
  SUBROUTINE MapAaToAc(    mesh, d_aa, d_ac)
  ...
  SUBROUTINE MapAaToAc_3D( mesh, d_aa, d_ac)
    ! Map data from the regular Aa mesh to the staggered Ac mesh    
  ...
  SUBROUTINE MapAcToAa(    mesh, d_ac, d_aa)
    ! Map data from the regular Aa mesh to the staggered Ac mesh    
  ...
  SUBROUTINE MapAcToAa_3D( mesh, d_ac, d_aa)
    ! Map data from the regular Aa mesh to the staggered Ac mesh    
