MODULE thermodynamics_module
  ...
  USE data_types_module,           ONLY: type_mesh, type_ice_model, type_climate_model, type_SMB_model
  ...
  USE mesh_derivatives_module,     ONLY: GetUpwindDerivativeVertex_3D, ApplyNeumannBoundary_3D
...
CONTAINS
  ...
  SUBROUTINE update_ice_temperature(mesh, ice, climate, SMB)
    ...
    ! Local variables:
    INTEGER                                            :: vi, k, i, ierr, status(MPI_STATUS_SIZE)
    ...
    ! Allocate and initialise the new temperature field
    CALL allocate_shared_memory_dp_2D( mesh%nV, C%nZ, Ti_new, wTi_new)
    Ti_new(mesh%v1:mesh%v2,:) = 0._dp
    CALL sync
    
    ! Calculate the 3D ice velocities and zeta derivatives required for solving the heat equation
    CALL solve_SIA_3D(               ice, mesh)
    CALL bottom_frictional_heating(  ice, mesh)
    CALL calculate_zeta_derivatives( ice, mesh)
    ...
    ice%Fr(mesh%v1:mesh%v2) = C%geothermal_heat_flux
    CALL sync
    ...
    DO vi = mesh%v1, mesh%v2
      ...
      DO k = 2, C%NZ-1
        ...
        CALL GetUpwindDerivativeVertex_3D(mesh, ice%U_3D, ice%V_3D, ice%Ti, vi, k, dTi_dx, dTi_dy)
...
!        delta(k) = ice%Ti(vi,k) / C%dt_thermo
      ...
      IF (ice%mask_shelf(vi) == 1 .OR. ice%mask_groundingline(vi) == 1) THEN
      ...
      ELSE
        ...
        alpha(C%NZ) = 1._dp
        beta (C%NZ) = -1._dp
        delta(C%NZ) = (C%zeta(C%NZ) - C%zeta(C%NZ-1)) * (C%geothermal_heat_flux + ice%frictional_heating(vi)) / (ice%dzeta_dz(vi) * ice%Ki(vi,C%NZ)) 
      ...
      IF (Ti_new(vi,C%NZ) >= ice%Ti_pmp(vi,C%NZ)) THEN
        Ti_new(vi,C%NZ) = MIN( ice%Ti_pmp(vi,C%NZ), ice%Ti(vi,C%NZ-1) - (C%zeta(C%NZ) - C%zeta(C%NZ-1)) * (C%geothermal_heat_flux + ice%frictional_heating(vi)) / (ice%dzeta_dz(vi) * ice%Ki(vi,C%NZ)))
      END IF
      
    END DO ! DO vi = mesh%v1, mesh%v2
    CALL sync
        
    CALL ApplyNeumannBoundary_3D( mesh, Ti_new, C%nZ)
    CALL sync
    
    ice%Ti(mesh%v1:mesh%v2,:) = Ti_new(mesh%v1:mesh%v2,:)
    CALL sync
    
    CALL deallocate_shared_memory( wTi_new)
    NULLIFY( Ti_new)
  ...
  SUBROUTINE replace_Ti_with_robin_solution( ice, climate, SMB, vi)
    ...
    thermal_conductivity_robin        = kappa_0_ice_conductivity * C%sec_per_year * EXP(-kappa_e_ice_conductivity * T0)           ! Thermal conductivity            [J m^-1 K^-1 y^-1]
    ...
    IF (ice%mask_sheet(vi) == 1 ) THEN
      ...
      IF (SMB%SMB_year(vi) > 0._dp) THEN    
        ...
        DO k = 1, C%nZ
          ...
          ice%Ti(vi,k) = Ts + SQRT(C%pi) / 2._dp * thermal_length_scale * bottom_temperature_gradient_robin * (erf1 - erf2)
        END DO
  ...
  SUBROUTINE bottom_frictional_heating(ice, mesh)
    ! Calculation of the frictional heating at the bottom due to sliding at the sheet/Gl - bedrock interface.
    
    USE parameters_module,           ONLY: ice_density, grav, q_plastic, u_threshold, delta_v
    ...
    DO vi = mesh%v1, mesh%v2
      IF (ice%mask_sheet(vi) == 1) THEN      
        beta_base = ice%tau_yield(vi) * ( (delta_v**2 + ice%U_SSA(vi)**2 + ice%V_SSA(vi)**2)**(0.5_dp * (q_plastic-1._dp)) ) / (u_threshold**q_plastic)
