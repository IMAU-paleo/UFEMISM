MODULE mesh_Delaunay_module
  ...
  USE mesh_help_functions_module,  ONLY: IsInTriangle, IsBoundarySegment, FindCircumcenter, FindContainingTriangle, &
                                         LineFromPoints, LineLineIntersection, PerpendicularBisectorFromLine, &
                                         EncroachesUpon, UpdateTriangleCircumcenter
  ...
  SUBROUTINE SplitTriangle( mesh, ti, p_new)
    ...
    CALL EncroachesUpon( mesh, p_new(1), p_new(2), isencroached, va, vb)
    IF (isencroached) THEN
      p = (mesh%V(va,:) + mesh%V(vb,:)) / 2._dp
      CALL SplitSegment( mesh, va, vb, p)
      RETURN
    END IF
    ...
    IF (p_new(1)<mesh%xmin) THEN
      IF (p_new(2)<mesh%ymin .OR. p_new(2)>mesh%ymax) THEN
        WRITE(0,*) 'SplitTriangle - ERROR - circumcenter lies way outside of grid!'
        STOP
      END IF
      ...
      END DO
      IF (v1==0) THEN
        WRITE(0,*) 'SplitTriangle - ERROR - circumcenter lies outside of grid; couldnt find segment to split!'
        STOP
      END IF
      p = (mesh%V(v1,:) + mesh%V(v2,:)) / 2._dp
      CALL SplitSegment( mesh, v1, v2, p)
      RETURN
    END IF
    ...
    t_old = ti
    CALL FindContainingTriangle( mesh, p_new, t_old)
    ...
    DO n = 1, 3
      ...
      p = mesh%V(vi,:)
      q = mesh%V(vj,:)
      CALL LineFromPoints(p, p_new, la, lb, lc)
      CALL LineFromPoints(p_new, q, le, lf, lg)

      CALL PerpendicularBisectorFromLine(p,p_new,la,lb,la2,lb2,lc2)
      CALL PerpendicularBisectorFromLine(p_new,q,le,lf,le2,lf2,lg2);

      CALL LineLineIntersection(la2,lb2,lc2,le2,lf2,lg2,cc2)

      IF (abs(cc2(1))>(mesh%xmax-mesh%xmin)*1000._dp .OR. abs(cc2(2))>(mesh%ymax-mesh%ymin)*1000._dp) THEN
        CALL SplitLine( mesh, vi, vj, p_new)
        RETURN
      END IF
    END DO
    ...
    DO n = 1, mesh%nC(p1)
      IF (mesh%C(p1,n)==p2) THEN
        mesh%C(p1,:) = [mesh%C(p1,1:n), mesh%nV, mesh%C(p1,n+1:mesh%nconmax-1)]
    ...
    DO n = 1, mesh%nC(p2)
      IF (mesh%C(p2,n)==p3) THEN
        mesh%C(p2,:) = [mesh%C(p2,1:n), mesh%nV, mesh%C(p2,n+1:mesh%nconmax-1)]
    ...
    DO n = 1, mesh%nC(p3)
      IF (mesh%C(p3,n)==p1) THEN
        mesh%C(p3,:) = [mesh%C(p3,1:n), mesh%nV, mesh%C(p3,n+1:mesh%nconmax-1)]
    ...
    DO n = 1, mesh%niTri(p1)
      IF (mesh%iTri(p1,n)==t_old) THEN
        mesh%iTri(p1,:) = [mesh%iTri(p1,1:n-1), t_new1, t_new3, mesh%iTri(p1,n+1:mesh%nconmax-1)]
    ...
    DO n = 1, mesh%niTri(p2)
      IF (mesh%iTri(p2,n)==t_old) THEN
        mesh%iTri(p2,:) = [mesh%iTri(p2,1:n-1), t_new2, t_new1, mesh%iTri(p2,n+1:mesh%nconmax-1)]
    ...
    DO n = 1, mesh%niTri(p3)
      IF (mesh%iTri(p3,n)==t_old) THEN
        mesh%iTri(p3,:) = [mesh%iTri(p3,1:n-1), t_new3, t_new2, mesh%iTri(p3,n+1:mesh%nconmax-1)]
    ...
    CALL UpdateTriangleCircumcenter( mesh, t_new1)
    CALL UpdateTriangleCircumcenter( mesh, t_new2)
    CALL UpdateTriangleCircumcenter( mesh, t_new3)
    ...
    ! list by FlipTrianglePairs, making sure the loop runs until all flip
    ...
    DO WHILE (nf>0)
      CALL FlipTrianglePairs( mesh, nf)
    END DO

  END SUBROUTINE SplitTriangle
  SUBROUTINE SplitLine( mesh, v1a, v2a, p_new)
    ...
    IF (.NOT. AreConnected) THEN
      WRITE(0,*) 'SplitLine - ERROR: trying to split a non-existing line!'
      STOP
    END IF

    ! If both of those are boundary vertices, the line is a Segment - refer
    ! to that function instead.
    IF (IsBoundarySegment(mesh,v1,v2)) THEN
     p = (mesh%V(v1,:) + mesh%V(v2,:)) / 2._dp
     CALL SplitSegment( mesh, v1, v2, p)
     RETURN
    END IF
    ...
    IF (t1==0 .OR. t2==0) THEN
      WRITE(0,*) 'SplitLine - ERROR: couldnt find two triangles containing both vertices!'
    END IF
    ...
    ! == Find circumcenters
    CALL UpdateTriangleCircumcenter( mesh, t1new1)
    CALL UpdateTriangleCircumcenter( mesh, t1new2)
    CALL UpdateTriangleCircumcenter( mesh, t2new1)
    CALL UpdateTriangleCircumcenter( mesh, t2new2)

    ! == Update inverse triangle matrix
    ! vo1
    DO n = 1, mesh%niTri(vo1)
      IF (mesh%iTri(vo1,n)==t1) THEN
        mesh%iTri(vo1,:) = [mesh%iTri(vo1,1:n-1), t1new2, t1new1, mesh%iTri(vo1,n+1:mesh%nconmax-1)]
    ...
    DO n = 1, mesh%niTri(vo2)
      IF (mesh%iTri(vo2,n)==t2) THEN
        mesh%iTri(vo2,:) = [mesh%iTri(vo2,1:n-1), t2new2, t2new1, mesh%iTri(vo2,n+1:mesh%nconmax-1)]
    ...
    DO n = 1, mesh%nC(vo1)
      IF (mesh%C(vo1,n)==v1) THEN
        mesh%C(vo1,:) = [mesh%C(vo1,1:n), mesh%nV, mesh%C(vo1,n+1:mesh%nconmax-1)]
    ...
    DO n = 1, mesh%nC(vo2)
      IF (mesh%C(vo2,n)==v2) THEN
        mesh%C(vo2,:) = [mesh%C(vo2,1:n), mesh%nV, mesh%C(vo2,n+1:mesh%nconmax-1)]
  ...
  SUBROUTINE SplitSegment( mesh, v1a, v2a, p_new)
    ...
    IF (t1==0) THEN
      WRITE(0,*) 'SplitSegment - ERROR: couldnt find triangle containing both vertices!'
      STOP
    END IF

    IF (mesh%edge_index(v1)==0 .OR. mesh%edge_index(v2)==0) THEN
      WRITE(0,*) 'SplitSegment - ERROR: segment isnt made up of boundary vertices!'
      STOP
    END IF
    ...
    ELSEIF (   mesh%edge_index(v1)==2) THEN
      ...
      ELSE
        WRITE(0,*) 'SplitSegment - ERROR: edge indices of v1 and v2 dont make sense!'
        STOP
      END IF
    ...
    ! == Update triangle Circumcenters
    CALL UpdateTriangleCircumcenter( mesh, tnew1)
    CALL UpdateTriangleCircumcenter( mesh, tnew2)

    ! == Update inverse triangle matrix
    ! vo
    DO n = 1, mesh%niTri(vo)
      IF (mesh%iTri(vo,n)==t1) THEN
        mesh%iTri(vo,:) = [mesh%iTri(vo,1:n-1), tnew1, tnew2, mesh%iTri(vo,n+1:mesh%nconmax-1)];
    ...
    DO n = 1, mesh%nC(vo)
      IF (mesh%C(vo,n)==v1) THEN
        mesh%C(vo,:) = [mesh%C(vo,1:n), mesh%nV, mesh%C(vo,n+1:mesh%nconmax-1)]
  ...
  END SUBROUTINE SplitSegment
  SUBROUTINE FlipTrianglePairs( mesh, nf)
    ...
    INTEGER,  DIMENSION(5)                          :: edgevals, edgevals_left, edgevals_right
     ...
     IF ((n1to2 .AND. .NOT. n2to1) .OR. (n2to1 .AND. .NOT. n1to2)) THEN
       WRITE(0,*) 'FlipTrianglePairs - ERROR: Somethings really wrong with the triangle connectivity matrix!'
       STOP
     END IF
     ...
     CALL NeedFlipping( mesh, t1, t2, FlipThem, vo1, vo2, v1, v2, t1nv1, t1nv2, t2nv1, t2nv2)
     ...
     DO n = 1, mesh%niTri(v1)
       IF (mesh%iTri(v1,n)==t2) THEN
         mesh%iTri(v1,:) = [mesh%iTri(v1,1:n-1), mesh%iTri(v1,n+1:mesh%nconmax), 0]
     ...
     DO n = 1, mesh%niTri(v2)
       IF (mesh%iTri(v2,n)==t1) THEN
         mesh%iTri(v2,:) = [mesh%iTri(v2,1:n-1), mesh%iTri(v2,n+1:mesh%nconmax), 0]
     ...
     DO n = 1, mesh%niTri(vo1)
       IF (mesh%iTri(vo1,n)==t1) THEN
         mesh%iTri(vo1,:) = [mesh%iTri(vo1,1:n), t2, mesh%iTri(vo1,n+1:mesh%nconmax-1)]
         mesh%niTri(vo1) = mesh%niTri(vo1)+1
         EXIT
       END IF
     END DO
     ...
     DO n = 1, mesh%niTri(vo2)
       IF (mesh%iTri(vo2,n)==t2) THEN
         mesh%iTri(vo2,:) = [mesh%iTri(vo2,1:n), t1, mesh%iTri(vo2,n+1:mesh%nconmax-1)]
         mesh%niTri(vo2) = mesh%niTri(vo2)+1
         EXIT
       END IF
     END DO
     ...
     DO n = 1, mesh%nC(v1)
       IF (mesh%C(v1,n)==v2) THEN
         mesh%C(v1,:) = [mesh%C(v1,1:n-1), mesh%C(v1,n+1:mesh%nconmax), 0]
     ...
     DO n = 1, mesh%nC(v2)
       IF (mesh%C(v2,n)==v1) THEN
         mesh%C(v2,:) = [mesh%C(v2,1:n-1), mesh%C(v2,n+1:mesh%nconmax), 0]
     ...
     DO n = 1, mesh%nC(vo1)
       IF (mesh%C(vo1,n)==v1) THEN
         mesh%C(vo1,:) = [mesh%C(vo1,1:n), vo2, mesh%C(vo1,n+1:mesh%nconmax-1)]
         mesh%nC(vo1) = mesh%nC(vo1)+1
         EXIT
       END IF
     END DO
     ...
     DO n = 1, mesh%nC(vo2)
       IF (mesh%C(vo2,n)==v2) THEN
         mesh%C(vo2,:) = [mesh%C(vo2,1:n), vo1, mesh%C(vo2,n+1:mesh%nconmax-1)]
         mesh%nC(vo2) = mesh%nC(vo2)+1
         EXIT
       END IF
     END DO
     ...
     ! == Update triangle circumcenters
    CALL UpdateTriangleCircumcenter( mesh, t1)
    CALL UpdateTriangleCircumcenter( mesh, t2)
   ...
   END SUBROUTINE FlipTrianglePairs
  SUBROUTINE NeedFlipping( mesh, t1, t2, isso, vo1, vo2, v1, v2, t1nv1, t1nv2, t2nv1, t2nv2)
...
!        norm2(mesh%V(vo1,:) - mesh%Tricc(t2,:)) < norm2(mesh%V(mesh%Tri(t2,1),:) - mesh%Tricc(t2,:)) - mesh%tol_dist) THEN
    ...
    r = mesh%V(v1,:)
    s = mesh%V(v2,:)
    IF  (IsInTriangle(p, q, s, r) .OR. &
         IsInTriangle(p, q, r, s)) THEN
      isso = .FALSE.
    END IF

  END SUBROUTINE NeedFlipping
  
  SUBROUTINE EnsureDelaunay( mesh)  
    ! Make sure a mesh meets the Delaunay criterion by inspecting, and, if necessary,
    ! flipping, all possible triangle pairs. No way to parallelise this, so VERY heavy.
    ! Currently not used anywhere, try to keep it that way!

    TYPE(type_mesh),                  INTENT(INOUT) :: mesh

    INTEGER, DIMENSION(:,:), ALLOCATABLE            :: Trip, TriMap
    INTEGER                                         :: nTrip, ti, tc, n1, n2, tii, nf

    IF (par%master) THEN
    
      ! First, construct a list of all unique triangle pairs, kind of like the
      ! Arakawa C mesh
      ALLOCATE( Trip(   mesh%nTri*3, 2))
      ALLOCATE( TriMap( mesh%nTri,   3))
      ...
      DO ti = 1, mesh%nTri
        ! Add all non-listed triangle connections to the list      
        DO n1 = 1, 3
          tc = mesh%TriC( ti,n1)
          
          IF (tc==0) CYCLE ! This neighbour triangle doesn't exist          
          IF (TriMap( ti,n1) ==1) CYCLE ! This triangle connection is already listed
          ...
          ! Mark the reverse connection as listed
          DO n2 = 1, 3
            IF (mesh%TriC( tc,n2) == ti) TriMap( tc,n2) = 1
          END DO
        END DO
      END DO ! DO ti = 1, mesh%nTri
      
      ! Go over all triangle connections, flip them if necessary
      DO tii = 1, nTrip
      
        nf = 1
        mesh%Triflip(1,:) = Trip( tii,:)
  ...
  END SUBROUTINE EnsureDelaunay
  SUBROUTINE EnsureDelaunay_old( mesh)  
    ! Make sure a mesh meets the Delaunay criterion by inspecting, and, if necessary,
    ! flipping, all possible triangle pairs. No way to parallelise this, so VERY heavy.
    ! Currently not used anywhere, try to keep it that way!
    ...
    INTEGER                                         :: vi, iti, ti1, ti2, n, nf

    IF (par%master) THEN
    
      DO vi = 1, mesh%nV
  
        ! Update the triangulation by performing the necessary flip operations
        ! mesh%Triflip(:,:) = 0
        nf = 0
  ...
  END SUBROUTINE EnsureDelaunay_old
