MODULE mesh_mapping_module
  ...
  SUBROUTINE create_remapping_arrays_mesh_grid( mesh, grid)
    ...
    INTEGER                                                :: i, j, k, il, iu, jl, ju
    REAL(dp)                                               :: xo_min, xo_max, yo_min, yo_max, A_overlap, A
    INTEGER,  DIMENSION(:,:  ), ALLOCATABLE                :: grid_nV_proc
    INTEGER,  DIMENSION(:,:,:), ALLOCATABLE                :: grid_vi_proc
    REAL(dp), DIMENSION(:,:,:), ALLOCATABLE                :: grid_w_g2m_proc
    REAL(dp), DIMENSION(:,:,:), ALLOCATABLE                :: grid_w_m2g_proc
    INTEGER,  DIMENSION(:    ), ALLOCATABLE                :: i_temp
    INTEGER,  DIMENSION(:    ), ALLOCATABLE                :: j_temp
    REAL(dp), DIMENSION(:    ), ALLOCATABLE                :: A_temp
    INTEGER                                                :: n_cells_in_vertex, nV, vii
    INTEGER,  DIMENSION(:,:  ), POINTER                    :: is_treated
    INTEGER                                                :: wis_treated
    INTEGER                                                :: ti, via, vib, vic
    REAL(dp), DIMENSION(2)                                 :: p, pa, pb, pc
    REAL(dp)                                               :: Atri, Aa, Ab, Ac, wa, wb, wc
    
    ! Determine maximum number of contributions
    nmax = 4 * CEILING( MAX( MAXVAL(mesh%A) / grid%dx**2, grid%dx**2 / MINVAL(mesh%A)))

    ! Allocate shared memory
    CALL allocate_shared_int_1D( mesh%nV,                       grid%map%mesh_nV,    grid%map%wmesh_nV   )
    CALL allocate_shared_int_2D( mesh%nV,          mesh%nC_mem, grid%map%mesh_i,     grid%map%wmesh_i    )
    CALL allocate_shared_int_2D( mesh%nV,          mesh%nC_mem, grid%map%mesh_j,     grid%map%wmesh_j    )
    CALL allocate_shared_dp_2D(  mesh%nV,          mesh%nC_mem, grid%map%mesh_w_g2m, grid%map%wmesh_w_g2m)
    CALL allocate_shared_dp_2D(  mesh%nV,          mesh%nC_mem, grid%map%mesh_w_m2g, grid%map%wmesh_w_m2g)
    CALL allocate_shared_int_2D( grid%nx, grid%ny,              grid%map%grid_nV,    grid%map%wgrid_nV   )
    CALL allocate_shared_int_3D( grid%nx, grid%ny, mesh%nC_mem, grid%map%grid_vi,    grid%map%wgrid_vi   )
    CALL allocate_shared_dp_3D(  grid%nx, grid%ny, mesh%nC_mem, grid%map%grid_w_g2m, grid%map%wgrid_w_g2m)
    CALL allocate_shared_dp_3D(  grid%nx, grid%ny, mesh%nC_mem, grid%map%grid_w_m2g, grid%map%wgrid_w_m2g)
    CALL allocate_shared_dp_2D(  grid%nx, grid%ny,              grid%map%grid_w_tot, grid%map%wgrid_w_tot)
    
    ! Allocate local memory (so some parallelisation can be done)
    ALLOCATE( grid_nV_proc(    grid%nx, grid%ny))
    ALLOCATE( grid_vi_proc(    grid%nx, grid%ny, mesh%nC_mem))
    ALLOCATE( grid_w_g2m_proc( grid%nx, grid%ny, mesh%nC_mem))
    ALLOCATE( grid_w_m2g_proc( grid%nx, grid%ny, mesh%nC_mem))
    
    grid_nV_proc    = 0
    grid_vi_proc    = 0
    grid_w_g2m_proc = 0._dp
    grid_w_m2g_proc = 0._dp

    ! Temporary index and area lists
    ALLOCATE( i_temp( nmax))
    ALLOCATE( j_temp( nmax))
    ALLOCATE( A_temp( nmax))
    ...
    DO vi = mesh%v1, mesh%v2
      ...
      DO j = jl, ju
        ...
        i_temp( n_cells_in_vertex) = i
        j_temp( n_cells_in_vertex) = j
        A_temp( n_cells_in_vertex) = A_overlap

      END DO ! DO j = jl, ju
      END DO ! DO i = il, iu
  
      ! Fill data into mapping arrays
      IF (n_cells_in_vertex <= mesh%nC_mem) THEN
        ! This mesh vertex has <= nC_mem grid cells contributing to it. List them on the mesh.
        
        grid%map%mesh_nV( vi) = n_cells_in_vertex
        DO vii = 1, n_cells_in_vertex
          grid%map%mesh_i(     vi,vii) = i_temp( vii)
          grid%map%mesh_j(     vi,vii) = j_temp( vii)
          grid%map%mesh_w_g2m( vi,vii) = A_temp( vii) / SUM( A_temp( 1:n_cells_in_vertex))
          grid%map%mesh_w_m2g( vi,vii) = A_temp( vii) / (grid%dx**2)
        END DO
        
      ELSE
        ! This mesh vertex has >  nC_mem grid cells contributing to it. List them on the grid.
        
        DO vii = 1, n_cells_in_vertex
          i = i_temp( vii)
          j = j_temp( vii)
          A = A_temp( vii)
          grid_nV_proc(     i,j   ) = grid_nV_proc( i,j) + 1
          nV = grid_nV_proc( i,j)
          grid_vi_proc(     i,j,nV) = vi
          grid_w_g2m_proc(  i,j,nV) = A / SUM( A_temp( 1:n_cells_in_vertex))
          grid_w_m2g_proc(  i,j,nV) = A / (grid%dx**2)
        END DO
        
      END IF ! IF (n_cells_in_vertex <= mesh%nC_mem) THEN

    END DO ! DO vi = mesh%v1, mesh%v2
    CALL sync
    
    ! Move data from process-local memory to shared memory (each process in turn)
    DO k = 0, par%n-1
      IF (k == par%i) THEN
        
        DO i = 1, grid%nx
        DO j = 1, grid%ny
          nV = grid%map%grid_nV( i,j)
          DO vii = 1, grid_nV_proc( i,j)
            grid%map%grid_vi(    i,j,nV + vii) = grid_vi_proc(    i,j,vii)
            grid%map%grid_w_g2m( i,j,nV + vii) = grid_w_g2m_proc( i,j,vii)
            grid%map%grid_w_m2g( i,j,nV + vii) = grid_w_m2g_proc( i,j,vii)
          END DO
          grid%map%grid_nV( i,j) = grid%map%grid_nV( i,j) + grid_nV_proc( i,j)
        END DO
        END DO
    ...
    ! Find grid cells that are not yet listed use trilinear interpolation for
    ! those. Also, normalise weights DO those that have been treated.
    
    CALL allocate_shared_int_2D( grid%nx, grid%ny, is_treated, wis_treated)
    
    IF (par%master) THEN
      DO vi = 1, mesh%nV
        DO vii = 1, grid%map%mesh_nV( vi)
          i = grid%map%mesh_i( vi,vii)
          j = grid%map%mesh_j( vi,vii)
          is_treated( i,j) = 1
          grid%map%grid_w_tot( i,j) = grid%map%grid_w_tot( i,j) + grid%map%mesh_w_m2g( vi,vii)
        END DO
      END DO
    ...
    DO j = 1, grid%ny
      IF (grid%map%grid_nV( i,j) > 0) THEN
        is_treated( i,j) = 1
        grid%map%grid_w_tot( i,j) = grid%map%grid_w_tot( i,j) + SUM(grid%map%grid_w_m2g( i,j,:))
      END IF
    END DO
    END DO
    CALL sync
    ...
    DO i = grid%i1, grid%i2
    DO j = 1, grid%ny
      IF (is_treated( i,j) == 0) THEN

        ! Find the mesh triangle containing this grid cell
        p = [MIN( mesh%xmax, MAX( mesh%xmin, grid%x(i) )), &
        ...
        Atri = ABS( cross2( (pb-pa), (pc-pa) ))
        Aa   = ABS( cross2( (pb-p ), (pc-p ) ))
        Ab   = ABS( cross2( (pc-p ), (pa-p ) ))
        Ac   = ABS( cross2( (pa-p ), (pb-p ) ))

        wa   = Aa / Atri
        wb   = Ab / Atri
        wc   = Ac / Atri

        ! List weights in mapping arrays
        grid%map%grid_nV(    i,j  ) = 3
        grid%map%grid_vi(    i,j,1) = via
        grid%map%grid_vi(    i,j,2) = vib
        grid%map%grid_vi(    i,j,3) = vic
        grid%map%grid_w_m2g( i,j,1) = wa
        grid%map%grid_w_m2g( i,j,2) = wb
        grid%map%grid_w_m2g( i,j,3) = wc
        grid%map%grid_w_tot( i,j  ) = 1
        
      END IF ! IF (is_treated( i,j) == 0) THEN
    END DO
    END DO
    
    ! Clean up after yourself
    DEALLOCATE( grid_nV_proc   )
    DEALLOCATE( grid_vi_proc   )
    DEALLOCATE( grid_w_g2m_proc)
    DEALLOCATE( grid_w_m2g_proc)
    DEALLOCATE( i_temp         )
    DEALLOCATE( j_temp         )
    DEALLOCATE( A_temp         )
    
    CALL deallocate_shared( wis_treated)
  ...
  SUBROUTINE map_grid2mesh_2D( mesh, grid, d_grid, d_mesh)
    ...
    ! First do vertices that contain fewer than nC_mem grid cells (data listed on mesh)
    DO vi = mesh%v1, mesh%v2
      d_mesh( vi) = 0._dp
      DO vii = 1, grid%map%mesh_nV( vi)
        i = grid%map%mesh_i(     vi,vii)
        j = grid%map%mesh_j(     vi,vii)
        w = grid%map%mesh_w_g2m( vi,vii)
        d_mesh( vi) = d_mesh( vi) + w * d_grid( i,j)
      END DO
    END DO
    CALL sync
    
    ! Then vertices that contain more than this (data listed on grid)
    IF (par%master) THEN
      DO i = 1, grid%nx
      DO j = 1, grid%ny
        DO vii = 1, grid%map%grid_nV( i,j)
          vi = grid%map%grid_vi(    i,j,vii)
          w  = grid%map%grid_w_g2m( i,j,vii)
          d_mesh( vi) = d_mesh( vi) + w * d_grid( i,j)
        END DO
      END DO
      END DO
  ...
  SUBROUTINE map_mesh2grid_2D( mesh, grid, d_mesh, d_grid)
    ...
    d_grid( grid%i1:grid%i2,:) = 0._dp
    CALL sync
    
    ! First vertices that contain more than this (data listed on grid)
    DO i = grid%i1, grid%i2
    DO j = 1, grid%ny
      DO vii = 1, grid%map%grid_nV( i,j)
        vi = grid%map%grid_vi(     i,j,vii)
        w  = grid%map%grid_w_m2g(  i,j,vii)
        d_grid( i,j) = d_grid( i,j) + w * d_mesh( vi)
      END DO
    END DO
    END DO
    CALL sync

    ! Then do vertices that contain fewer than nC_mem grid cells (data listed on mesh)
    IF (par%master) THEN
      DO vi = 1, mesh%nV
        DO vii = 1, grid%map%mesh_nV( vi)
          i = grid%map%mesh_i(     vi,vii)
          j = grid%map%mesh_j(     vi,vii)
          w = grid%map%mesh_w_m2g( vi,vii)
          d_grid( i,j) = d_grid( i,j) + w * d_mesh( vi)
        END DO
      END DO
    END IF ! IF (par%master) THEN
    ...
    DO i = grid%i1, grid%i2
    DO j = 1, grid%ny
      d_grid( i,j) = d_grid( i,j) / grid%map%grid_w_tot( i,j)
    END DO
    END DO
    CALL sync
  ...
  SUBROUTINE map_mesh2grid_2D_min( mesh, grid, d_mesh, d_grid)
    ...
    ! Local variables:
    INTEGER                                                :: i,j,vii,vi,ierr
    REAL(dp)                                               :: d_max
    
    ! Determine maximum value over entire mesh
    d_max = MAXVAL( d_mesh( mesh%v1:mesh%v2))
    CALL MPI_ALLREDUCE( MPI_IN_PLACE, d_max, 1, MPI_DOUBLE_PRECISION, MPI_MAX, MPI_COMM_WORLD, ierr)
    
    d_grid( grid%i1:grid%i2,:) = d_max
    CALL sync
  ...
  SUBROUTINE deallocate_remapping_arrays_mesh_grid( grid)
    ...
    CALL deallocate_shared( grid%map%wmesh_nV   )
    CALL deallocate_shared( grid%map%wmesh_i    )
    CALL deallocate_shared( grid%map%wmesh_j    )
    CALL deallocate_shared( grid%map%wmesh_w_g2m)
    CALL deallocate_shared( grid%map%wmesh_w_m2g)
    CALL deallocate_shared( grid%map%wgrid_nV   )
    CALL deallocate_shared( grid%map%wgrid_vi   )
    CALL deallocate_shared( grid%map%wgrid_w_g2m)
    CALL deallocate_shared( grid%map%wgrid_w_m2g)
    CALL deallocate_shared( grid%map%wgrid_w_tot)
