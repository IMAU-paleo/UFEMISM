...
!    ! Write a 2-D data field defined on a mesh to a NetCDF file variable on a grid
...
!
!    ! Check if this file has the desired dimensions
!    CALL inquire_dim_multiple_options( filename, field_name_options_x     , nx, id_dim_x)
!    IF (id_dim_x    == -1) CALL crash('no valid x dimension could be found in file "' // TRIM( filename) // '"!')
!    CALL inquire_dim_multiple_options( filename, field_name_options_y     , ny, id_dim_y)
!    IF (id_dim_y    == -1) CALL crash('no valid y dimension could be found in file "' // TRIM( filename) // '"!')
!    CALL inquire_dim_multiple_options( filename, field_name_options_time  , nt, id_dim_time)
!    IF (id_dim_time == -1) CALL crash('no valid time dimension could be found in file "' // TRIM( filename) // '"!')
!
!    ! Look for the desired variable in the file
!    CALL inquire_var_multiple_options( filename, field_name_options, id_var)
!    IF (id_var == -1) CALL crash('no match for variable "' // TRIM( field_name_options) // '" could be found in file "' // TRIM( filename) // '"!')
!
!    ! Check if this variable has the correct type and number of dimensions
...
!
!      ! Check variable type
!      IF (var_type /= NF90_DOUBLE) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" is not of type NF90_DOUBLE!')
!
!      ! Check number of dimensions
!      IF (ndims_of_var /= 3) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // &
!        '" has wrong number of dimensions (expected 3, found {int_01})', int_01 = ndims_of_var)
!
!      ! Check dimensions
!      IF (.NOT. ANY( dims_of_var == id_dim_x   )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have an x dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_y   )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a y dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_time)) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a time dimension!')
!
!    END IF ! IF (par%master) THEN
!    CALL close_netcdf_file( ncid)
!
!    ! Write data to the variable
!    CALL write_var_dp_2D( filename, id_var, d_grid, ti = nt)
!
...
!    ! Write a 2-D monthly data field defined on a mesh to a NetCDF file variable on a grid
...
!    INTEGER                                            :: wd_grid
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nx, ny, nm, nt, id_dim_x, id_dim_y, id_dim_month, id_dim_time
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!    CALL inquire_dim_multiple_options( filename, field_name_options_y     , ny, id_dim_y)
!    IF (id_dim_y    == -1) CALL crash('no valid y dimension could be found in file "' // TRIM( filename) // '"!')
!    CALL inquire_dim_multiple_options( filename, field_name_options_month , nm, id_dim_month)
!    IF (id_dim_month == -1) CALL crash('no valid month dimension could be found in file "' // TRIM( filename) // '"!')
!    IF (nm /= 12) CALL crash('unexpected number of months = {int_01} found in file "' // TRIM( filename) // '"!', int_01 = nm)
!    CALL inquire_dim_multiple_options( filename, field_name_options_time  , nt, id_dim_time)
!    IF (id_dim_time == -1) CALL crash('no valid time dimension could be found in file "' // TRIM( filename) // '"!')
!
!    ! Look for the desired variable in the file
...
!
!      ! Check number of dimensions
!      IF (ndims_of_var /= 4) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // &
!        '" has wrong number of dimensions (expected 4, found {int_01})', int_01 = ndims_of_var)
!
!      ! Check dimensions
!      IF (.NOT. ANY( dims_of_var == id_dim_x    )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have an x dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_y    )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a y dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_month)) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a month dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_time )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a time dimension!')
!
!    END IF ! IF (par%master) THEN
!    CALL close_netcdf_file( ncid)
!
!    ! Write data to the variable
!    CALL write_var_dp_3D( filename, id_var, d_grid, ti = nt)
!
!    ! Clean up after yourself
!    CALL deallocate_grid(                             grid)
!    CALL deallocate_remapping_operators_mesh2grid(    grid)
...
!    ! Write a 3-D data field defined on a mesh to a NetCDF file variable on a grid
...
!    INTEGER                                            :: wd_grid
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nx, ny, nz, nt, id_dim_x, id_dim_y, id_dim_zeta, id_dim_time
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!    CALL inquire_dim_multiple_options( filename, field_name_options_y     , ny, id_dim_y)
!    IF (id_dim_y    == -1) CALL crash('no valid y dimension could be found in file "' // TRIM( filename) // '"!')
!    CALL inquire_dim_multiple_options( filename, field_name_options_zeta  , nz, id_dim_zeta)
!    IF (id_dim_zeta == -1) CALL crash('no valid zeta dimension could be found in file "' // TRIM( filename) // '"!')
!    IF (nz /= C%nz) CALL crash('unexpected nz = {int_01} found in file "' // TRIM( filename) // '"!', int_01 = nz)
!    CALL inquire_dim_multiple_options( filename, field_name_options_time  , nt, id_dim_time)
!    IF (id_dim_time == -1) CALL crash('no valid time dimension could be found in file "' // TRIM( filename) // '"!')
!
!    ! Look for the desired variable in the file
...
!      IF (.NOT. ANY( dims_of_var == id_dim_x   )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have an x dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_y   )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a y dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_zeta)) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a zeta dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_time)) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a time dimension!')
!
!    END IF ! IF (par%master) THEN
!    CALL close_netcdf_file( ncid)
...
!
!      ! Check number of dimensions
!      IF (ndims_of_var /= 2) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // &
!        '" has wrong number of dimensions (expected 2, found {int_01})', int_01 = ndims_of_var)
!
!      ! Check dimensions
!      IF (.NOT. ANY( dims_of_var == id_dim_x   )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have an x dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_y   )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a y dimension!')
...
!    INTEGER                                            :: wd_grid
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nx, ny, nm, id_dim_x, id_dim_y, id_dim_month
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!    INTEGER                                            :: wd_grid
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nx, ny, nz, id_dim_x, id_dim_y, id_dim_zeta
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nV, nt, id_dim_vi, id_dim_time
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!
!    ! Check if this file has the desired dimensions
!    CALL inquire_dim_multiple_options( filename, field_name_options_dim_nV, nV, id_dim_vi)
!    IF (id_dim_vi   == -1) CALL crash('no valid vertex dimension could be found in file "' // TRIM( filename) // '"!')
!    IF (nV /= mesh%nV) CALL crash('mesh size in file "' // TRIM( filename) // '" does not match model mesh size!')
!    CALL inquire_dim_multiple_options( filename, field_name_options_time  , nt, id_dim_time)
!    IF (id_dim_time == -1) CALL crash('no valid time dimension could be found in file "' // TRIM( filename) // '"!')
!
!    ! Look for the desired variable in the file
...
!
!      ! Check variable type
!      IF (var_type /= NF90_INT) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" is not of type NF90_INT!')
!
!      ! Check number of dimensions
!      IF (ndims_of_var /= 2) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // &
!        '" has wrong number of dimensions (expected 2, found {int_01})', int_01 = ndims_of_var)
!
!      ! Check dimensions
!      IF (.NOT. ANY( dims_of_var == id_dim_vi  )) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a vertex dimension!')
!      IF (.NOT. ANY( dims_of_var == id_dim_time)) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a time dimension!')
!
!    END IF ! IF (par%master) THEN
!    CALL close_netcdf_file( ncid)
!
!    ! Write data to the variable
!    CALL write_var_int_1D( filename, id_var, d, ti = nt)
!
!    ! Finalise routine path
!    CALL finalise_routine( routine_name)
!
...
!
!    ! Write data to the variable
!    CALL write_var_dp_1D( filename, id_var, d, ti = nt)
!
!    ! Finalise routine path
!    CALL finalise_routine( routine_name)
!
...
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nV, nm, nt, id_dim_vi, id_dim_month, id_dim_time
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!
!    ! Write data to the variable
!    CALL write_var_dp_2D( filename, id_var, d, ti = nt)
!
!    ! Finalise routine path
!    CALL finalise_routine( routine_name)
!
...
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nV, nz, nt, id_dim_vi, id_dim_zeta, id_dim_time
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!
!      ! Check number of dimensions
!      IF (ndims_of_var /= 1) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // &
!        '" has wrong number of dimensions (expected 1, found {int_01})', int_01 = ndims_of_var)
!
!      ! Check dimensions
!      IF (.NOT. ANY( dims_of_var == id_dim_vi)) CALL crash('variable "' // TRIM( var_name) // '" in file "' // TRIM( filename) // '" does not have a vertex dimension!')
!
...
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nV, nm, id_dim_vi, id_dim_month
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!    INTEGER                                            :: ncid, id_var
!    INTEGER                                            :: nV, nz, id_dim_vi, id_dim_zeta
!    CHARACTER(LEN=256)                                 :: var_name
!    INTEGER                                            :: var_type
!    INTEGER                                            :: ndims_of_var
!    INTEGER, DIMENSION(NF90_MAX_VAR_DIMS)              ::  dims_of_var
...
!    CALL inquire_dim_multiple_options( filename, field_name_options_time, nt, id_dim_time)
!    IF (nt < 0 .OR. id_dim_time == -1) CALL crash('no valid time dimension could be found in file "' // TRIM( filename) // '"!')
!
...
!    ! Write time
!    nt = nt + 1
!    CALL write_var_dp_0D( filename, id_var_time, time, nt)
!
!    ! Finalise routine path
!    CALL finalise_routine( routine_name)
!
...
!
!    ! lon
!    CALL create_variable( filename, get_first_option_from_list( field_name_options_lon), NF90_DOUBLE, (/ id_dim_x, id_dim_y /), id_var_lon)
!    CALL write_var_dp_2D( filename, id_var_lon, grid%lon)
!    ! lat
!    CALL create_variable( filename, get_first_option_from_list( field_name_options_lat), NF90_DOUBLE, (/ id_dim_x, id_dim_y /), id_var_lat)
...
!    INTEGER                                            :: nx, ny, nt, id_dim_x, id_dim_y, id_dim_time, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    INTEGER                                            :: nx, ny, nm, nt, id_dim_x, id_dim_y, id_dim_month, id_dim_time, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    INTEGER                                            :: nx, ny, nz, nt, id_dim_x, id_dim_y, id_dim_zeta, id_dim_time, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    ! Add a 2-D variable to an existing NetCDF file with an x/y-grid, without a time dimension
!
!    IMPLICIT NONE
!
!    ! In/output variables:
...
!    INTEGER                                            :: nx, ny,  id_dim_x, id_dim_y, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    ! Add a 2-D monthly variable to an existing NetCDF file with an x/y-grid, without a time dimension
!
!    IMPLICIT NONE
!
!    ! In/output variables:
...
!    INTEGER                                            :: nx, ny, nm,  id_dim_x, id_dim_y, id_dim_month, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    ! Add a 3-D variable to an existing NetCDF file with an x/y-grid, without a time dimension
!
!    IMPLICIT NONE
!
!    ! In/output variables:
...
!    INTEGER                                            :: nx, ny, nz, id_dim_x, id_dim_y, id_dim_zeta, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!
!    ! R
!    CALL add_field_mesh_dp_2D_notime( filename, 'R'  , long_name = 'Resolution', units = 'm')
!    CALL write_to_field_multiple_options_mesh_dp_2D_notime( filename, mesh, 'R'  , mesh%R  )
!    ! A
!    CALL add_field_mesh_dp_2D_notime( filename, 'A'  , long_name = 'Voronoi cell area', units = 'm^2')
!    CALL write_to_field_multiple_options_mesh_dp_2D_notime( filename, mesh, 'A'  , mesh%A  )
!    ! lon
!    CALL add_field_mesh_dp_2D_notime( filename, 'lon', long_name = 'Longitude', units = 'degrees east')
!    CALL write_to_field_multiple_options_mesh_dp_2D_notime( filename, mesh, 'lon', mesh%lon)
!    ! lat
!    CALL add_field_mesh_dp_2D_notime( filename, 'lat', long_name = 'Latitude' , units = 'degrees north')
!    CALL write_to_field_multiple_options_mesh_dp_2D_notime( filename, mesh, 'lat', mesh%lat)
!
!    ! Finalise routine path
!    CALL finalise_routine( routine_name)
!
...
!    INTEGER                                            :: nV, nt, id_dim_vi, id_dim_time, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    INTEGER                                            :: nV, nm, nt, id_dim_vi, id_dim_month, id_dim_time, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    INTEGER                                            :: nV, nz, nt, id_dim_vi, id_dim_zeta, id_dim_time, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    ! Add a 2-D variable to an existing NetCDF file with a mesh, without a time dimension
!
!    IMPLICIT NONE
!
!    ! In/output variables:
...
!    ! Add a 2-D monthly variable to an existing NetCDF file with a mesh, without a time dimension
!
!    IMPLICIT NONE
!
!    ! In/output variables:
...
!    INTEGER                                            :: nV, nm, id_dim_vi, id_dim_month, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!    ! Add a 3-D variable to an existing NetCDF file with a mesh, without a time dimension
!
!    IMPLICIT NONE
!
!    ! In/output variables:
...
!    INTEGER                                            :: nV, nz, id_dim_vi, id_dim_zeta, id_var
!
!    ! Add routine to path
!    CALL init_routine( routine_name)
!
...
!  ! =================
!
!  SUBROUTINE NetCDF_output_test( region, region_name)
