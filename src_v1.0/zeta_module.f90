...
CONTAINS
  ...
  SUBROUTINE calculate_zeta_derivatives(ice, mesh)
...
!    ALLOCATE(p_zeta%b_zeta_plus(1:C%NZ-2))
!    ALLOCATE(p_zeta%c_zeta_plus(1:C%NZ-2))
!    ALLOCATE(p_zeta%d_zeta_plus(1:C%NZ-2))
  
!    p_zeta%a_x  = -1._dp / (2._dp * C%dx_ant)
!    p_zeta%c_x  =  1._dp / (2._dp * C%dx_ant)
!    p_zeta%a_y  = -1._dp / (2._dp * C%dy_ant)
!    p_zeta%c_y  =  1._dp / (2._dp * C%dy_ant)
!    p_zeta%a_xx =  1._dp / C%dx_ant**2
!    p_zeta%b_xx = -2._dp / C%dx_ant**2
!    p_zeta%c_xx =  1._dp / C%dx_ant**2
!    p_zeta%a_yy =  1._dp / C%dy_ant**2
!    p_zeta%b_yy = -2._dp / C%dy_ant**2
!    p_zeta%c_yy =  1._dp / C%dy_ant**2
...
!    DO k = 1, C%NZ-2
!      p_zeta%b_zeta_plus(k) = -(b_k(k) + d_k(k)) / (b_k(k) * d_k(k))
!      p_zeta%c_zeta_plus(k) =            d_k(k)  / (b_k(k) * (d_k(k) - b_k(k)))   
!      p_zeta%d_zeta_plus(k) =            b_k(k)  / (d_k(k) * (b_k(k) - d_k(k)))          
!    END DO
...
!    p_zeta%z_x_minus =  1._dp / (2._dp * C%dx_ant)
!    p_zeta%a_x_minus = -2._dp /          C%dx_ant
!    p_zeta%b_x_minus =  3._dp / (2._dp * C%dx_ant)

!    p_zeta%b_x_plus = -3._dp / (2._dp * C%dx_ant)
!    p_zeta%c_x_plus =  2._dp /          C%dx_ant
!    p_zeta%d_x_plus = -1._dp / (2._dp * C%dx_ant)

!    p_zeta%z_y_minus =  1._dp / (2._dp * C%dy_ant)
!    p_zeta%a_y_minus = -2._dp /          C%dy_ant
!    p_zeta%b_y_minus =  3._dp / (2._dp * C%dy_ant)

!    p_zeta%b_y_plus = -3._dp / (2._dp * C%dy_ant)
!    p_zeta%c_y_plus =  2._dp /          C%dy_ant
!    p_zeta%d_y_plus = -1._dp / (2._dp * C%dy_ant)
